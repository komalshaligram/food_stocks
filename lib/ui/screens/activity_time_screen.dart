...
class ActivityTimeScreenWidget extends StatelessWidget {
                ...
                child: SingleChildScrollView(
                  ...
                  child: Padding(
                    ...
                    child: Column(mainAxisAlignment: MainAxisAlignment.start, children: [
                      10.height,
                      fromUntilTimeWidget(context),
                      operationTimeListWidget(state),
                      40.height,
                      saveButtonWidget(context, state),
                      20.height,
                      skipButtonWidget(context, state),
                      20.height,
                    ]),
                  ),
                ),
  ...
  PreferredSizeWidget appBarWidget(BuildContext context, ActivityTimeState state) => AppBar(
  ...
  Widget fromUntilTimeWidget(BuildContext context) => Row(mainAxisAlignment: MainAxisAlignment.start, children: [
        50.height,
        SizedBox(
        ...
        SizedBox(
          ...
          child: Text(AppLocalizations.of(context)!.from_time, style: AppStyles.rkRegularTextStyle(size: AppConstants.smallFont, color: AppColors.textColor)),
        ),
        18.width,
        SizedBox(
          width: getScreenWidth(context) >= 700 ? getScreenWidth(context) * 0.27 : getScreenWidth(context) * 0.25,
          height: getScreenHeight(context) >= 1000 ? 30 : 20,
          child: Text(AppLocalizations.of(context)!.until_time, style: AppStyles.rkRegularTextStyle(size: AppConstants.smallFont, color: AppColors.textColor)),
        ),
        10.height,
      ]);

  Widget operationTimeListWidget(ActivityTimeState state) => state.operationTimeList.isNotEmpty
      ? ListView.builder(
          ...
          itemBuilder: (context, index) {
            return Padding(
              ...
              child: ActivityTimeRow(dayString: state.operationTimeList[index].dayString, rowIndex: index),
            );
          })
      : const SizedBox();
                      ...
                      return state.operationTimeList.isNotEmpty
                          ? Padding(
                              ...
                              child: Row(children: [
                                index == 0
                                    ? Expanded(
                                        child: Text(dayString, style: AppStyles.rkRegularTextStyle(size: AppConstants.smallFont, color: AppColors.textColor)),
                                      )
                                    : Expanded(child: Container()),
                                SizedBox(width: getScreenWidth(context) < 380 ? getScreenWidth(context) * 0.001 : getScreenWidth(context) * 0.03),
                                TimeContainer(
                                  openingIndex: 1,
                                  index: index,
                                  rowIndex: rowIndex,
                  ...
                  builder: (BuildContext c1) {
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        borderRadius: const BorderRadius.only(topLeft: Radius.circular(AppConstants.radius_20), topRight: Radius.circular(AppConstants.padding_20)),
                      ),
