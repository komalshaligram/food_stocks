...
class BasketScreenWidget extends StatelessWidget {
                      ...
                      state.isShimmering
                          ...
                          : state.basketProductList.isNotEmpty
                              ...
                              : !state.isShimmering && state.basketProductList.isEmpty
                                  ? Expanded(
                                      child: noDataWidget(AppLocalizations.of(context)!.cart_empty),
                                    )
                                  : const BasketScreenShimmerWidget(),
  ...
  Widget appBarWidget(BuildContext context, BasketState state) => state.basketProductList.isNotEmpty
      ? Container(
          ...
          child: Row(children: [
            Expanded(
              child: InkWell(
                onTap: () {
                  deleteDialog(context: context, updateClearString: AppStrings.clearString, listIndex: 0, cartProductId: '', totalAmount: 0.0);
                },
                child: Row(children: [
                  SvgPicture.asset(AppImagePath.delete, colorFilter: ColorFilter.mode(AppColors.redColor, BlendMode.srcIn)),
                  5.width,
                  Text(AppLocalizations.of(context)!.empty, style: AppStyles.rkRegularTextStyle(size: AppConstants.font_14, color: AppColors.redColor)),
                ]),
              ),
            ),
            Expanded(
  ...
  Widget basketRow(String title, String amount, {bool isTitle = false, double fontSize = 20}) {
    return Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      Text(title, style: AppStyles.rkRegularTextStyle(size: AppConstants.font_20, color: AppColors.blackColor, fontWeight: FontWeight.w700)),
      Text(
        amount,
        ...
        background: Container(
          ...
          child: Padding(
            ...
            child: GestureDetector(
              ...
              child: SvgPicture.asset(AppImagePath.delete, colorFilter: ColorFilter.mode(AppColors.whiteColor, BlendMode.srcIn), height: 30, width: 30),
            ),
          ),
        ),
  ...
  void deleteDialog({required BuildContext context, required String updateClearString, required String cartProductId, required int listIndex, double? totalAmount}) {
    BasketBloc bloc = context.read<BasketBloc>();
    showDialog(
        context: context,
        builder: (context1) => BlocProvider.value(
  ...
  void showProductDetails({
    ...
    context.read<BasketBloc>().add(BasketEvent.getProductDetailsEvent(context: context, productId: cartProductId, isBarcode: isBarcode, productListIndex: productListIndex));
