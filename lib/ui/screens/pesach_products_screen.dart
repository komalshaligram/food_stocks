...
class PesachProductsScreenWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<PesachProductsBloc, PesachProductsState>(
      ...
      child: BlocBuilder<PesachProductsBloc, PesachProductsState>(builder: (context, state) {
        return Scaffold(
  ...
  Widget buildSupplierProducts({
    ...
    return Container(
      decoration: BoxDecoration(color: AppColors.whiteColor, borderRadius: const BorderRadius.all(Radius.circular(AppConstants.radius_10)), boxShadow: [
  ...
  showMinQtyConfirmDialog(BuildContext context, String productId, String minBox, bool? isMixedSale, List? sameSaleProducts) {
    ...
    showDialog(
      ...
      builder: (dialogContext) => BlocProvider.value(
        value: context.read<PesachProductsBloc>(),
        child: BlocBuilder<PesachProductsBloc, PesachProductsState>(builder: (context1, state) {
          String mixedSale = '';
          if (isMixedSale!) {
  ...
  void showMinMaxQtyConfirmDialog({
    ...
    showDialog(
      ...
      builder: (dialogContext) => BlocProvider.value(
        value: bloc,
        child: BlocBuilder<PesachProductsBloc, PesachProductsState>(builder: (context1, state) {
          final mixedSaleText = (isMixedSale ?? false) ? AppStrings.minSaleText(context, minBox) : AppStrings.otherSaleText(context, minBox);
