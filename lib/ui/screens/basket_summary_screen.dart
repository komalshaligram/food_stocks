...
class BasketSummaryScreenWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<BasketSummaryBloc, BasketSummaryState>(
      ...
      child: BlocBuilder<BasketSummaryBloc, BasketSummaryState>(builder: (context, state) {
                  ...
                  child: Material(
                    ...
                    child: Container(
                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        SizedBox(height: 80, width: 130, child: Lottie.asset('assets/images/super_market.json', width: 50, height: 50, fit: BoxFit.fill)),
                        10.height,
                        Text(
                ...
                actions: [
                  ...
                  InkWell(
                    ...
                    child: Container(
                      ...
                      child: Text(AppLocalizations.of(context)!.closeText, style: AppStyles.rkRegularTextStyle(color: AppColors.mainColor, size: AppConstants.smallFont)),
                    ),
                  ),
                ]),
          );
  ...
  void paymentOptionPopupOne(BasketSummaryState state, BuildContext context, BasketSummaryBloc bloc, {bool isFromRemovePopUp = false}) {
    showDialog(
        context: context,
        builder: (context1) {
          bool c = state.paymentTypesList.any((e) => e == AppStrings.wallet);
  ...
  void paymentOptionPopupTwo(BasketSummaryState state, BuildContext context, BasketSummaryBloc bloc, {bool isFromRemovePopUp = false}) {
    showDialog(
        context: context,
        builder: (context1) {
          bool c = state.paymentTypesList.any((e) => e == AppStrings.wallet);
  ...
  void paymentOptionPopupThree(BasketSummaryState state, BuildContext context, BasketSummaryBloc bloc, {bool isFromRemovePopUp = false}) {
    showDialog(
        context: context,
        builder: (context1) {
          return CustomOneButtonDialog(
          ...
          CustomButtonWidget(
            ...
            onPressed: () async {
              if (state.tempList[index].draftReturnExists!) {
                await showDialog(
                  context: context,
                  builder: (_) => CallAgentDialog(language: state.language, id: state.tempList[index].suppliers?.id ?? '', index: index, bloc: bloc),
                );
              } else {
...
class CallAgentDialog extends StatelessWidget {
  ...
  const CallAgentDialog({Key? key, required this.language, required this.id, required this.index, required this.bloc}) : super(key: key);
              ...
              InkWell(
                ...
                child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Text(AppLocalizations.of(context)!.view_return, style: AppStyles.rkRegularTextStyle(size: AppConstants.smallFont, color: AppColors.whiteColor)),
                  ]),
                ),
              ),
              InkWell(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(gradient: AppColors.appMainGradientColor, borderRadius: BorderRadius.circular(AppConstants.radius_5)),
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Text(AppLocalizations.of(context)!.send_any_way, style: AppStyles.rkRegularTextStyle(size: AppConstants.smallFont, color: AppColors.whiteColor)),
                  ]),
                ),
              ),
            ]),
