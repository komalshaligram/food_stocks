  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => MyAccountingCardBloc()
        ..add(MyAccountingCardEvent.getClientInvoicesFromRivchitDataEvent(
            context: context))
        ..add(MyAccountingCardEvent.getClientRefundInvoicesFromRivchitDataEvent(
            context: context)),
...
class MyAccountingCardScreenContent extends StatelessWidget {
  ...
  void _refreshAccountingData(BuildContext context) {
    context.read<MyAccountingCardBloc>().add(
        MyAccountingCardEvent.getClientInvoicesFromRivchitDataEvent(
            context: context));
    context.read<MyAccountingCardBloc>().add(
        MyAccountingCardEvent.getClientRefundInvoicesFromRivchitDataEvent(
            context: context));
  }
  ...
  Future<void> _showDatePickerBottomSheet(BuildContext context) async {
    ...
    DateTime tempFrom = isInvoicesTab
        ? (state.invoicesFrom ?? defaultFromDate)
        : (state.refundsFrom ?? defaultFromDate);
    DateTime tempTo = isInvoicesTab
        ? state.invoicesTo ?? DateTime.now()
        : state.refundsTo ?? DateTime.now();
                    ...
                    Row(children: [
                      Expanded(
                        child: InkWell(
                          ...
                          child: _buildDateTile(_formatDate(tempFrom),
                      ...
                      Expanded(
                        child: InkWell(
                          ...
                          child: _buildDateTile(_formatDate(tempTo),
                              AppLocalizations.of(context)!.select_date),
                        ),
                      ),
                    ]),
                      ...
                      Expanded(
                        child: GestureDetector(
                          ...
                          child: Container(
                            ...
                            padding:
                                const EdgeInsets.all(AppConstants.padding_8),
                      ...
                      Expanded(
                        child: GestureDetector(
                          onTap: () {
                            ...
                            if (isInvoicesTab) {
                              bloc.add(
                                  MyAccountingCardEvent.updateInvoicesDateRange(
                                      from: tempFrom,
                                      to: tempTo,
                                      context: context));
                            } else {
                              bloc.add(
                                  MyAccountingCardEvent.updateRefundsDateRange(
                                      from: tempFrom,
                                      to: tempTo,
                                      context: context));
                            }
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<MyAccountingCardBloc, MyAccountingCardState>(
        ...
        builder: (context, state) {
          final isInvoicesTab = state.selectedTabIndex == 0;
          final fromDate =
              isInvoicesTab ? state.invoicesFrom : state.refundsFrom;
          ...
          return PopScope(
            ...
            child: Scaffold(
              ...
              appBar: PreferredSize(
                  preferredSize:
                      const Size.fromHeight(AppConstants.containerHeight_60),
                  child: AppBar(
                    backgroundColor: AppColors.pageColor,
                          ...
                          Expanded(
                            child: AnimatedSwitcher(
                              ...
                              child: state.selectedTabIndex == 0
                                  ? _invoicesTab(context, state)
                                  : _refundsTab(context, state),
                            ),
                          ),
                        ]),
                ),
        ...
        children: List.generate(tabs.length, (index) {
          ...
          return Expanded(
            child: GestureDetector(
              onTap: () {
                context
                    .read<MyAccountingCardBloc>()
                    .add(MyAccountingCardEvent.changeTab(index));
              },
          ...
          Widget itemOne(MyAccountingCardState state) => Row(
                  ...
                  children: [
                    Column(
                        ...
                        children: [
                          titleText(
                              context, AppLocalizations.of(context)!.invoice),
                          GestureDetector(
                            onTap: () {
                              final refundInvoiceData = MyCardInvoice(
                                invoiceLink:
                                    state.invoiceCardList[index].invoiceLink,
                                invoiceNumber:
                                    state.invoiceCardList[index].invoiceNumber,
                                invoiceAmount:
                                    state.invoiceCardList[index].invoiceAmount,
                                paymentStatus:
                                    state.invoiceCardList[index].paymentStatus,
                                invoiceDate:
                                    state.invoiceCardList[index].invoiceDate,
                                dueDate: state.invoiceCardList[index].dueDate,
                                orderNumber:
                                    state.invoiceCardList[index].orderNumber,
                                orderId: state.invoiceCardList[index].orderId,
                                rivchitApiKey:
                                    state.invoiceCardList[index].rivchitApiKey,
                                cardNumber:
                                    state.invoiceCardList[index].cardNumber,
                                paymentMethod:
                                    state.invoiceCardList[index].paymentMethod,
                              );

                              final invoiceData = Invoice(
                                invoiceLink: refundInvoiceData.invoiceLink,
                                invoiceNumber:
                                    refundInvoiceData.invoiceNumber.toString(),
                            ...
                            child: invoiceOrderNumberWidget(state
                                .invoiceCardList[index].invoiceNumber
                                .toString()),
                          ),
                        ]),
                    ...
                    Expanded(
                      child: Column(
                          ...
                          children: [
                            ...
                            subTitleValueText(
                                context,
                                (state.invoiceCardList[index].dueDate ?? '')
                                        .isNotEmpty
                                    ? state.invoiceCardList[index].dueDate!
                                        .substring(0, 10)
                                    : '---'),
                          ...
                          GestureDetector(
                            onTap: () async {
                              ...
                              preferences.setOrderId(
                                  productOrderId: state
                                      .invoiceCardList[index].orderId
                                      .toString());
                              Navigator.push(
                                  context,
                                  PageRouteBuilder(
                                      pageBuilder: (context, animation,
                                          ...
                                          ProductDetailsScreen(
                                            ...
                                            orderNumber: state
                                                .invoiceCardList[index]
                                                .orderNumber
                                                .toString(),
                                            orderId: state
                                                .invoiceCardList[index].orderId
                                                .toString(),
                                            isNavigateToProductDetailString:
                                                true,
                                            isFromBasket: false,
                                          ),
                            ...
                            child:
                                state.invoiceCardList[index].orderNumber == null
                                    ? const Text('---')
                                    : invoiceOrderNumberWidget(state
                                        .invoiceCardList[index].orderNumber
                                        .toString()),
                          ),
                        ]),
                  ),
                        ...
                        children: [
                          ...
                          Directionality(
                              ...
                              child: subTitleValueText(
                                  context,
                                  formatSignedNumber(state
                                      .invoiceCardList[index].invoiceAmount))),
                        ]),
                  ),
                ]);
          }
          ...
          return Container(
            ...
            child: Column(children: [
              ...
              state.invoiceCardList[index].paymentStatus == AppStrings.openText
                  ? GestureDetector(
                      onTap: () {
                        if (state.invoiceCardList[index].cardNumber != null &&
                            state.invoiceCardList[index]
                                    .customerCreditcardToken !=
                                null) {
                          Navigator.pushNamed(
                              context, RouteDefine.invoicePaymentScreen.name,
                              arguments: {
                                AppStrings.invoiceData:
                                    state.invoiceCardList[index]
                              }).then((value) {
            ...
            child: Column(
                ...
                children: [
                  _refundInvoiceWidget(
                    ...
                    index,
                    context,
                    state.refundInvoicesCardList[index].invoiceNumber
                        .toString(),
                    state.statusList,
                  ...
                  state.refundInvoicesCardList[index].status.toString() !=
                      ...
                      : const IgnorePointer(),
                  (state.refundInvoicesCardList[index].refundedOnOrders!
                              .isEmpty &&
                          state.refundInvoicesCardList[index].status
                                  .toString() !=
                              AppStrings.openText)
                      ? const IgnorePointer()
                      : titleText(
                  ...
                  state.refundInvoicesCardList[index].refundedOnInvoice!.isEmpty
                      ...
                      : _refundedOnInvoiceWidget(
                          ...
                          state.refundInvoicesCardList[index].invoiceNumber
                              .toString(),
                          state
                              .refundInvoicesCardList[index].refundedOnInvoice!,
                        ),
                  const DividerWidget(height: 20.0),
                  _totalRefundWidget(
                      context,
                      state.refundInvoicesCardList[index].totalAmount
                          .toString(),
                      state.refundInvoicesCardList[index].remainingAmount
                          .toString()),
                ]),
          );
        });
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              invoiceNumber.isEmpty
                  ...
                  : GestureDetector(
                      onTap: () {
                        Navigator.pushNamed(
                            context, RouteDefine.refundPdfScreen.name,
                            arguments: {
  ...
  Widget _refundedOnOrderWidget(List<RefundInvoiceCommon> invoicesList,
          int index, String invoiceNumber, List<StatusData> statusList) =>
      ListView.builder(
              ...
              child: Row(
                  ...
                  children: [
                                ...
                                GestureDetector(
                                  onTap: () {
                                    final invoiceData = Invoice(
                                      ...
                                      dueDate: inv.dueDate,
                                      invoiceAdjustAmount: double.tryParse(
                                              inv.invoiceAdjustAmount ?? '0') ??
                                          0.0,
                                      status: inv.status,
                    ...
                    Expanded(
                        child: titleGreenText(
                            context,
                            formatSignedNumber(inv.invoiceAdjustAmount),
                            widgets.TextDirection.ltr)),
                    2.height,
  ...
  Widget _totalRefundWidget(
          BuildContext context, String totalAmount, String remainingAmount) =>
