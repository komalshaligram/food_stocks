...
class CompanyProductsScreenWidget extends StatelessWidget {
            ...
            child: SafeArea(
              child: NotificationListener<ScrollNotification>(
                  child: Stack(children: [
                    Column(children: [
                      100.height,
                      if (state.categoryList.isNotEmpty) categoryChipRow(context, state),
                      if (state.selectedCategoryId.isNotEmpty && state.subCategoryList.isNotEmpty) subCategoryChipRow(context, state),
                      Expanded(
  ...
  Widget categoryChipRow(BuildContext context, CompanyProductsState state) {
    return SizedBox(
      width: getScreenWidth(context),
      height: 44,
      child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final category = state.categoryList[index - 1];
            return buildFilterChip(
                context: context,
  ...
  void _onCategoryTap(BuildContext context, String categoryId) {
    context.read<CompanyProductsBloc>().add(CompanyProductsEvent.selectCategoryEvent(context: context, categoryId: categoryId));
  }

  Widget subCategoryChipRow(BuildContext context, CompanyProductsState state) {
    return SizedBox(
      width: getScreenWidth(context),
      height: 44,
      child: ListView.builder(
  ...
  void _onSubCategoryTap(BuildContext context, String subCategoryId) {
    context.read<CompanyProductsBloc>().add(CompanyProductsEvent.selectSubCategoryEvent(context: context, subCategoryId: subCategoryId));
  }
                        ...
                        onQuantityIncreaseTap: () {
                          if (!(state.searchList[index].isMixedSale ?? false) &&
                            ...
                            showMinMaxQtyConfirmDialog(
                                context: context, productId: state.searchList[index].searchId, index: index, productListIndex: 0, isIncrease: false);
                          }
                        },
                                    ...
                                    child: Column(children: [
                                      CommonProductDetailsWidget(
                                          ...
                                          addToOrderTap: () {
                                            ...
                                            if (!isMixedSale &&
                                              ...
                                              context
                                                  ...
                                                  .add(CompanyProductsEvent.addToCartProductEvent(context: context1, productId: productId));
                                            } else {
                                          ...
                                          onQuantityChanged: (quantity) {
                                            context
                                                .read<CompanyProductsBloc>()
                                                .add(CompanyProductsEvent.updateQuantityOfProduct(context: context1, quantity: quantity));
                                          },
                                          onQuantityIncreaseTap: () {
                                            context
                                                .read<CompanyProductsBloc>()
                                                .add(CompanyProductsEvent.increaseQuantityOfProduct(context: context1));
                                          },
                                          onQuantityDecreaseTap: () {
                                            if (state.productStockList[state.productListIndex][state.productStockUpdateIndex].quantity > 1) {
                                              context
                                                  .read<CompanyProductsBloc>()
                                                  .add(CompanyProductsEvent.decreaseQuantityOfProduct(context: context1));
                                            }
                                          },
                                          onCloseTap: () {
                                            context
  ...
  void showMinMaxQtyConfirmDialog(
      ...
      int? initialQuantity}) {
    final stockState = context.read<CompanyProductsBloc>().state;
  ...
  Future<void> _openSalePromotionSheet(BuildContext context, String productId, {int? initialQuantity, bool closeParentBeforeOpen = false}) async {
    ...
    if (!context.mounted) return;
    bloc.add(CompanyProductsEvent.applyCartQuantitiesEvent(cartQuantities: cartMap));
