...
class WayOfPaymentScreenWidget extends StatelessWidget {
                ...
                if (state.isUpdate) {
                  ...
                  if (state.selectRadioTile == 0 && state.isEnablePayment) {
                    Navigator.pushNamed(context, RouteDefine.bankInfoScreen.name,
                        arguments: {AppStrings.termsConditionParamString: state.termsReqModel});
                  } else {
  ...
  Widget _buildRadio(bool selected) {
    return Container(
        ...
        height: 22,
        decoration:
            BoxDecoration(shape: BoxShape.circle, border: Border.all(color: selected ? AppColors.mainColor : AppColors.lightGreyColor, width: 2)),
        child: selected
            ? Center(child: Container(width: 12, height: 12, decoration: BoxDecoration(shape: BoxShape.circle, color: AppColors.mainColor)))
            : null);
  }
  ...
  Widget _buildSecureNote(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        ...
        10.width,
        Expanded(
          child: Text(AppLocalizations.of(context)!.payment_secure_note,
