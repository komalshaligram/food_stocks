...
Widget totalAmountCard(BasketState state, BuildContext context, BasketBloc bloc) {
  ...
  return Container(
      ...
      child: Column(children: [
        ...
        state.isIncludedVat
            ? basketRow(AppLocalizations.of(context)!.total_price_with_vat, formattedTotal, isTitle: true)
            : basketRow(AppLocalizations.of(context)!.total, formattedTotal, isTitle: true),
        const DividerWidget(height: 10.0),
        5.height,
...
String formattedBasketGrandTotal(BasketState state) {
  ...
  return formatNumber(value: state.isIncludedVat ? total.toString() : total.toStringAsFixed(2), local: AppStrings.hebrewLocal);
}
