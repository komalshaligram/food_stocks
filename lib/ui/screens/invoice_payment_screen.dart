...
class InvoicePaymentScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
        create: (context) => InvoicePaymentBloc()..add(InvoicePaymentEvent.getStatusDataEvent(context: context)),
        child: InvoicePaymentScreenWidget(invoiceData: invoiceData, creditCardNumber: creditCardNumber));
  }
}

class InvoicePaymentScreenWidget extends StatefulWidget {
  const InvoicePaymentScreenWidget({super.key, required this.invoiceData, required this.creditCardNumber});
  final MyCardInvoice invoiceData;
  final String? creditCardNumber;

  @override
  State<InvoicePaymentScreenWidget> createState() => _InvoicePaymentScreenWidgetState();
}

class _InvoicePaymentScreenWidgetState extends State<InvoicePaymentScreenWidget> {
  String? updatedCreditCardNumber;

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<InvoicePaymentBloc, InvoicePaymentState>(builder: (context, state) {
        ...
        return Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.center, children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            GestureDetector(
                ...
                child: invoiceOrderNumberWidget(invoiceDataList.invoiceNumber.toString())),
          ]),
          invoiceDataList.paymentMethod != null
              ? Container(
                  ...
                  child: Text(
                      invoiceDataList.paymentMethod.toString() == AppStrings.wallet
                          ? AppLocalizations.of(context)!.payment_wallet
                          : invoiceDataList.paymentMethod.toString() == AppStrings.creditCard
                              ? AppLocalizations.of(context)!.payment_credit_card
                              : invoiceDataList.paymentMethod.toString() == AppStrings.bankTransfer
              ...
              : 0.width,
          invoiceDataList.paymentStatus != null ? getPaymentStatusWidget(invoiceDataList.paymentStatus!, context) : 0.width
        ]);
      }
        ...
        return Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              subTitleValueText(context, (invoiceDataList.invoiceDate ?? '').isNotEmpty ? invoiceDataList.invoiceDate!.substring(0, 10) : '---')
            ]),
          ),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              titleText(context, AppLocalizations.of(context)!.due_date),
              subTitleValueText(context, (invoiceDataList.dueDate ?? '').isNotEmpty ? invoiceDataList.dueDate!.substring(0, 10) : '---')
            ]),
          ),
        ]);
      }

      Widget itemThree(InvoicePaymentState state, MyCardInvoice invoiceDataList) {
        return Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              GestureDetector(
                  onTap: () async {
                    ...
                    preferences.setOrderId(productOrderId: invoiceDataList.orderId.toString());
                    Navigator.push(
                        context,
                        PageRouteBuilder(
                            pageBuilder: (context, animation, secondaryAnimation) => ProductDetailsScreen(
                                statusList: state.statusList,
                                orderNumber: invoiceDataList.orderNumber.toString(),
                                orderId: invoiceDataList.orderId.toString(),
                  ...
                  child: invoiceDataList.orderNumber == null ? const Text('---') : invoiceOrderNumberWidget(invoiceDataList.orderNumber.toString())),
            ]),
          ),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Directionality(
                  textDirection: widgets.TextDirection.ltr, child: subTitleValueText(context, formatSignedNumber(invoiceDataList.payableAmount))),
            ]),
          ),
        ]);
      }
          ...
          body: SafeArea(
            child: Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                15.height,
                titleText(context, AppLocalizations.of(context)!.credit_card_details),
                15.height,
                Container(
                  padding: const EdgeInsets.all(AppConstants.padding_8),
                  decoration: BoxDecoration(
                  ...
                  child: Column(children: [
                    ...
                    Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      subTitleValueText(context, AppLocalizations.of(context)!.credit_card_number),
                      subTitleValueText(context, ' : '),
                      subTitleValueText(context, '********'),
                      subTitleValueText(
                          context,
                          widget.invoiceData.cardNumber == null
                              ? (updatedCreditCardNumber ?? widget.creditCardNumber ?? '')
                              : widget.invoiceData.cardNumber.toString())
                    ]),
                    10.height,
                    CustomTextIconButtonWidget(
                        title: AppLocalizations.of(context)!.change_credit_card,
                        onPressed: () async {
                          final result = await Navigator.pushNamed(context, RouteDefine.creditCardDetailsScreen.name, arguments: {
                          ...
                          if (!mounted) return;
                          if (result is Map && result[AppStrings.creditCardNumberString] != null) {
                            setState(() {
                              updatedCreditCardNumber = result[AppStrings.creditCardNumberString].toString();
                            });
                          }
                        })
                  ]),
          ...
          bottomSheet: Container(
            ...
            child: Padding(
              ...
              child: CustomButtonWidget(
                  buttonText: AppLocalizations.of(context)!.making_pay_with_credit_card.toUpperCase(),
                  bGColor: AppColors.mainColor,
                  isLoading: state.isLoading,
                  onPressed: () {
                    context.read<InvoicePaymentBloc>().add(InvoicePaymentEvent.addCreditCardEvent(
                        context: context,
