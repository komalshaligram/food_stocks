    ...
    return BlocProvider(
        create: (context) => PlanogramProductBloc()
          ..add(PlanogramProductEvent.getPlanogramProductsEvent(
              planogram: args?[AppStrings.planogramProductsParamString] ?? const PlanogramDatum(), context: context))
...
class PlanogramProductScreenWidget extends StatelessWidget {
  ...
  Widget listViewWidget(BuildContext context, PlanogramProductState state) => ListView.builder(
        ...
        itemBuilder: (context, index) => CommonSaleListView(
            context: context,
            discountedPrice:
                state.planogramProductList[index].sale!.isSale! ? double.parse(state.planogramProductList[index].sale!.salePrice.toString()) : 0.0,
                    ...
                    return SearchItemWidget(
                        ...
                        onTap: () async {
                          ...
                          if (state.searchList[index].searchType == SearchTypes.sale ||
                              state.searchList[index].searchType == SearchTypes.product && !state.isGuestUser) {
                            showProductDetails(
                                context: context,
                                      ...
                                      CommonProductDetailsWidget(
                                          ...
                                          addToOrderTap: () {
                                            context
                                                ...
                                                .add(PlanogramProductEvent.addToCartProductEvent(context: context1, productId: productId));
                                          },
                                          ...
                                          productPrice: state.productStockList[state.productListIndex][state.productStockUpdateIndex].totalPrice *
                                              state.productStockList[state.productListIndex][state.productStockUpdateIndex].quantity *
                                          ...
                                          onQuantityChanged: (quantity) {
                                            context
                                                .read<PlanogramProductBloc>()
                                                .add(PlanogramProductEvent.updateQuantityOfProduct(context: context1, quantity: quantity));
                                          },
                                          onQuantityIncreaseTap: () {
                                            context
                                                .read<PlanogramProductBloc>()
                                                .add(PlanogramProductEvent.increaseQuantityOfProduct(context: context1));
                                          },
                                          onQuantityDecreaseTap: () {
                                            if (state.productStockList[state.productListIndex][state.productStockUpdateIndex].quantity > 1) {
                                              context
                                                  .read<PlanogramProductBloc>()
                                                  .add(PlanogramProductEvent.decreaseQuantityOfProduct(context: context1));
                                            }
                                          },
                                          onCloseTap: () {
                                            Map<dynamic, dynamic>? args = ModalRoute.of(context)?.settings.arguments as Map?;
