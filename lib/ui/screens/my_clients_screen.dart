...
class MyClientsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
        create: (context) => MyClientsBloc()..add(MyClientsEvent.getAgentClientsListEvent(context: context)), child: const MyClientsScreenWidget());
  }
}
...
class MyClientsScreenWidget extends StatelessWidget {
              ...
              child: Stack(children: [
                state.isShimmering
                    ...
                    : Column(children: [
                        ...
                        if (state.filteredClientsList.isNotEmpty) _buildClientList(context, state) else Expanded(child: _buildEmptyState(context))
                      ]),
                if (state.isLoading)
                  Positioned.fill(
                    child: ColoredBox(
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: AppColors.shadowColor.withValues(alpha: 0.06), blurRadius: 12, offset: const Offset(0, 2))]),
        child: TextField(
            ...
            decoration: InputDecoration(
                ...
                focusedBorder: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: AppColors.mainColor.withValues(alpha: 0.4))),
                filled: true,
        ...
        child: ListView.separated(
          ...
          itemBuilder: (context, index) => AnimationConfiguration.staggeredList(
            ...
            child: SlideAnimation(
              verticalOffset: 24,
              child: FadeInAnimation(child: _buildClientCard(context, state, clientsList[index])),
            ),
          ),
        ),
      ),
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if ((client.storeRepresentativeName ?? '').isNotEmpty) ...[
              4.height,
              Text(client.storeRepresentativeName ?? '',
          ...
          _buildActionChip(
              ...
              onTap: () {
                context.read<MyClientsBloc>().add(MyClientsEvent.switchAccountEvent(
                    context: context, clientsId: client.id!, clientName: client.storeName ?? '', businessName: client.storeRepresentativeName));
              }),
          8.height,
          _buildActionChip(
              label: l10n.clients_set_minimum, isPrimary: false, onTap: () => supplierListDialog(context: context, state: state, client: client))
        ]),
      ]),
    );
  }
              ...
              builder: (context1, scrollController) {
                return SafeArea(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(borderRadius: const BorderRadius.vertical(top: Radius.circular(20)), color: AppColors.pageColor),
                                    ...
                                    return Material(
                                      ...
                                      child: InkWell(
                                        onTap: () => _onSupplierSelected(
                                            sheetContext: sheetContext, parentContext: context, client: client, supplier: supplier),
                                        borderRadius: BorderRadius.circular(14),
                                        child: Container(
                                          padding: const EdgeInsets.all(12),
                                          decoration: BoxDecoration(color: AppColors.whiteColor, borderRadius: BorderRadius.circular(14), boxShadow: [
                                            BoxShadow(
                                                color: AppColors.shadowColor.withValues(alpha: 0.06), blurRadius: 12, offset: const Offset(0, 2)),
                                          ]),
                                          child: Row(children: [
                                            Container(
                                              decoration: BoxDecoration(
                                                  border: Border.all(color: AppColors.lightBorderColor), borderRadius: BorderRadius.circular(12)),
                                              padding: const EdgeInsets.all(4),
                                              child: ClipRRect(
                                                borderRadius: BorderRadius.circular(8),
                                                child: Image.network("${AppUrlEndPoints.baseFileUrl}${supplier.logo}",
                                                    height: 40, width: 40, fit: BoxFit.contain, loadingBuilder: (context, child, loadingProgress) {
                                                  if (loadingProgress?.cumulativeBytesLoaded != loadingProgress?.expectedTotalBytes) {
                                                    return CommonShimmerWidget(child: Container(height: 40, width: 40, color: AppColors.pageColor));
                                                  }
                                                  return child;
  ...
  void _onSupplierSelected(
      {required BuildContext sheetContext, required BuildContext parentContext, required AgentStore client, required Supplier supplier}) {
    Navigator.of(sheetContext).pop();
    Future.delayed(const Duration(milliseconds: 200), () {
      confirmationDialog(context: parentContext, clientId: client.id!, supplierId: supplier.id!, isMinimum: supplier.isNoMinimum!);
    });
  }
            ...
            child: Directionality(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(color: AppColors.shadowColor.withValues(alpha: 0.12), blurRadius: 24, offset: const Offset(0, 8))]),
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  Container(
                    padding: const EdgeInsets.all(14),
                    decoration: BoxDecoration(color: AppColors.mainColor.withValues(alpha: 0.12), shape: BoxShape.circle),
                    child: Directionality(
                        textDirection: TextDirection.ltr, child: Icon(Icons.help_outline_rounded, color: AppColors.mainColor, size: 30)),
                  ),
                  16.height,
                  Text(AppLocalizations.of(context)!.are_you_sure,
                      textAlign: TextAlign.center, style: AppStyles.rkBoldTextStyle(size: AppConstants.font_17, color: AppColors.blackColor)),
                  12.height,
                  Text(message,
                      textAlign: TextAlign.center,
                      style: AppStyles.rkRegularTextStyle(
                  ...
                  24.height,
                  _dialogButton(AppLocalizations.of(context)!.yes, () {
                    Navigator.of(dialogContext).pop();
                    Future.delayed(const Duration(milliseconds: 100), () {
                      context.read<MyClientsBloc>().add(MyClientsEvent.updateAgentClientsNoMinimumEvent(
                          context: context, clientsId: clientId, supplierId: supplierId, isNoMinimum: isMinimum ? false : true));
                    });
                  }),
                  10.height,
                  _dialogButton(AppLocalizations.of(context)!.no, () => Navigator.of(dialogContext).pop(), outlined: true)
                ]),
              ),
            ),
          );
