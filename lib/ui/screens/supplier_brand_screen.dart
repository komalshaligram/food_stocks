...
class SupplierBrandScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => SupplierBrandBloc()..add(SupplierBrandEvent.initEvent(brandList: brandList)),
      child: SupplierBrandScreenWidget(supplierId: supplierId, supplierName: supplierName, brandList: brandList),
    );
  }
}

class SupplierBrandScreenWidget extends StatelessWidget {
  const SupplierBrandScreenWidget({super.key, required this.supplierId, required this.supplierName, required this.brandList});
  ...
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<SupplierBrandBloc, SupplierBrandState>(builder: (context, state) {
      return Scaffold(
        backgroundColor: AppColors.pageColor,
        appBar: PreferredSize(
          preferredSize: const Size.fromHeight(AppConstants.appBarHeight),
          child: CommonAppBar(bgColor: AppColors.pageColor, title: AppLocalizations.of(context)?.brands ?? '', iconData: Icons.arrow_back_ios_sharp, onTap: () => Navigator.pop(context)),
        ),
        body: SafeArea(
          child: SmartRefresher(
            ...
            onRefresh: () {
              context.read<SupplierBrandBloc>().add(SupplierBrandEvent.refreshEvent(brandList: brandList));
            },
            child: state.isShimmering
                ? const CompanyScreenShimmerWidget()
                : state.brandsList.isEmpty
                    ? Center(child: Text(AppLocalizations.of(context)?.companies_not_available ?? ''))
                    : GridView.builder(
                          ...
                          return buildSupplierBrandListItem(
                              ...
                              onTap: () {
                                Navigator.pushNamed(context, RouteDefine.supplierBrandProductsScreen.name, arguments: {
                                  ...
                                  AppStrings.brandNameText: item.brandName ?? '',
  ...
  Widget buildSupplierBrandListItem({required BuildContext context, required String brandLogo, required String brandName, required VoidCallback onTap}) {
    return Container(
          ...
          Expanded(
            child: brandLogo.isNotEmpty
                ? CachedNetworkImage(
                    ...
                    errorWidget: (_, __, ___) => Image.asset(AppImagePath.imageNotAvailable5, fit: BoxFit.cover),
                  )
