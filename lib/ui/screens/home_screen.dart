                                          ...
                                          child: ListView.builder(
                                              ...
                                              itemBuilder: (context, index) =>
                                                  CommonProductItemWidget(
                                                      ...
                                                      width: 140,
                                                      productImage:
                                                          state.recommendedProductsList[index].mainImage ??
                                                              '',
                                                      productName:
                                                          state.recommendedProductsList[index].productName ??
                                                              '',
                                                      totalSaleCount: state
                                                              .recommendedProductsList[index]
                                                              .totalSale ??
                                                          0,
                                                      price: state.recommendedProductsList[index].productPrice?.toDouble() ?? 0.0,
                                                      onButtonTap: () {
                                                        debugPrint(
                                                            'recommendedProductsList id: ${state.recommendedProductsList[index].id}');
                                                        showProductDetails(
                                                          context: context,
                                                          productId: state
                                                                  .recommendedProductsList[
                          ...
                          CommonSearchWidget(
                            ...
                            onSearch: (String search) {
                              ...
                              if (search.length > 1) {
                                bloc.add(HomeEvent.changeCategoryExpansion(
                                    isOpened: true));
                                bloc.add(HomeEvent.globalSearchEvent(
                                    context: context));
                              }
                            },
                                ...
                                : ListView.builder(
                                    ...
                                    itemBuilder: (listViewContext, index) {
                                      return _buildSearchItem(
                                          lowStock: state
                                              .searchList[index].lowStock
                                              .toString(),
                                          numberOfUnits: state
                                              .searchList[index].numberOfUnits,
                                          priceOfBox: state
                                              .searchList[index].priceOfBox,
                                          productStock: state
          ...
          if (message.data.isNotEmpty) {
            ...
            if (data != null) {
              ...
              if (imageUrl.isNotEmpty) {
                final response = await DioClient(context)
                    .get(path: AppUrls.baseFileUrl + imageUrl.toString());
                Directory dir;
              ...
              PushNotificationService().showNotification(
                ...
                title,
                body,
                PushNotificationService().channel!.id,
                PushNotificationService().channel!.name,
                PushNotificationService().channel!.description ?? '',
                android?.smallIcon ?? '',
              );
              PushNotificationService().showImage(fileName);
              PushNotificationService().manageNavigation(
                true,
                _mainPage ?? '',
                _subPage ?? '',
                _id ?? '',
