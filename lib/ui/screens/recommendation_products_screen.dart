...
class RecommendationProductsScreen extends StatefulWidget {
  ...
  @override
  State<RecommendationProductsScreen> createState() =>
      _RecommendationProductsScreenState();
}

class _RecommendationProductsScreenState
    extends State<RecommendationProductsScreen> {
  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    RecommendationProductsBloc().add(
...
class RecommendationProductsScreenWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    RecommendationProductsBloc bloc =
        context.read<RecommendationProductsBloc>();
    return BlocListener<RecommendationProductsBloc,
        ...
        return Scaffold(
              ...
              trailingWidget: GestureDetector(
                  onTap: () {
                    context.read<RecommendationProductsBloc>().add(
                        const RecommendationProductsEvent.getGridListView());
                  },
          ...
          body: FocusDetector(
            onFocusGained: () {
              ...
              bloc.add(RecommendationProductsEvent.getPermissionList(
                  context: context));
            },
            child: SafeArea(
                                    ...
                                    onRefresh: () {
                                      context
                                          ...
                                          .add(RecommendationProductsEvent
                                              .refreshListEvent(
                                                  context: context));
                                      bloc.add(const RecommendationProductsEvent
                                          .getPreferencesDataEvent());
                                    },
                                    child: SingleChildScrollView(
                                      physics: state.recommendationProductsList
                                              .isEmpty
  ...
  void _updateQuantity(
      ...
      required RecommendationProductsState state,
      required int index}) {
    final product = state.recommendationProductsList[index];

    context
        .read<RecommendationProductsBloc>()
        .add(RecommendationProductsEvent.updateListQuantityOfProduct(
          context: context,
  ...
  void _addToCart(
    ...
    context
        .read<RecommendationProductsBloc>()
        .add(RecommendationProductsEvent.addToCartListProductEvent(
          context: context,
  ...
  void _increaseQuantity(
    ...
    if (minQty <= quantity + 1) {
      context
          .read<RecommendationProductsBloc>()
          .add(RecommendationProductsEvent.increaseListQuantityOfProduct(
            context: context,
            productListIndex: 1,
            productStockUpdateIndex: index,
            productSupplierIds: product.supplierId.toString(),
  ...
  void _decreaseQuantity(
    ...
    if (minQty <= quantity - 1) {
      context
          .read<RecommendationProductsBloc>()
          .add(RecommendationProductsEvent.decreaseListQuantityOfProduct(
            context: context,
            productListIndex: 1,
            productStockUpdateIndex: index,
            productSupplierIds: product.supplierId.toString(),
  ...
  Widget gridViewWidget(
      BuildContext context, RecommendationProductsState state) {
    return GridView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return CommonProductSaleItemWidget(
              ...
              description: parse(productSaleData.sale?.saleDescription ?? '')
                      .body
                      ?.text ??
                  '',
              discountedPrice:
                  double.tryParse(productSaleData.sale?.salePrice ?? '') ?? 0.0,
              ...
              onQuantityChanged: () =>
                  _updateQuantity(context: context, state: state, index: index),
              onQuantityIncreaseTap: () => _increaseQuantity(
                  context: context, state: state, index: index),
              onQuantityDecreaseTap: () => _decreaseQuantity(
                  context: context, state: state, index: index),
              onButtonTap: () {
                showProductDetails(
                  context: context,
    ...
    return ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return CommonSaleListView(
            ...
            productName: productSaleData.productName ?? '',
            price:
                double.tryParse(productSaleData.productPrice.toString()) ?? 0.0,
            quantity: productStockData.quantity,
            minQuantity: productSaleData.sale?.saleMinQuantity,
            maxQuantity: productSaleData.sale?.saleMaxQuantity,
            isMixedSale: productSaleData.sale?.isMixedSale,
            ...
            onQuantityIncreaseTap: () =>
                _increaseQuantity(context: context, state: state, index: index),
            onQuantityDecreaseTap: () =>
                _decreaseQuantity(context: context, state: state, index: index),
            onButtonTap: () {
              showProductDetails(
                context: context,
                productId: productSaleData.id ?? '',
  ...
  Widget searchWidget(BuildContext context, RecommendationProductsBloc bloc,
          RecommendationProductsState state) =>
      CommonSearchWidget(
          onCloseTap: () {
          ...
          onFilterTap: () {
            bloc.add(
                const RecommendationProductsEvent.changeCategoryExpansion());
          },
          onSearchTap: () {
          ...
          onSearch: (String search) {
            if (search.length > 1) {
              bloc.add(
                  const RecommendationProductsEvent.changeCategoryExpansion(
                      isOpened: true));
              bloc.add(RecommendationProductsEvent.globalSearchEvent(
                  context: context));
            }
          },
                          ...
                          onQuantityIncreaseTap: () {
                            if (int.parse(
                                    state.searchList[index].saleMinQuantity ??
                                        '0') <=
                                state.productStockList[0][index].quantity + 1) {
                              context.read<RecommendationProductsBloc>().add(
                                      RecommendationProductsEvent
                              ...
                              showMinMaxQtyConfirmDialog(
                                context: context,
                                productId: state.searchList[index].searchId,
                                minBox: state.searchList[index].saleMinQuantity
                                    .toString(),
                                index: index,
  ...
  void showProductDetails({
    ...
    context
        .read<RecommendationProductsBloc>()
        .add(RecommendationProductsEvent.getProductDetailsEvent(
          context: context,
          productId: productId,
                                ...
                                : SingleChildScrollView(
                                    ...
                                    child: Column(children: [
                                      CommonProductDetailsWidget(
                                        ...
                                        productPrice: (state.productDetails
                                                    .first.sale?.isSale ??
                                                false)
                                        ...
                                        productQuantity: state
                                            .productStockList[
                                                ...
                                                [state.productStockUpdateIndex]
                                            .quantity,
                                        isMixedSale: state.productDetails.first
                                            .sale!.isMixedSale,
                                        recommendedRetailConsumerPricerOffer:
                                            state.clubAgentId ==
                                                    AppStrings.clubAgentIdText
                                                ? state.productDetails.first.sale
                                                            ?.isSale ==
                                                        true
                                                    ? state.productDetails.first
  ...
  showMinQtyConfirmDialog(BuildContext context, String productId, String minBox,
    ...
    showDialog(
      ...
      builder: (dialogContext) => BlocProvider.value(
        value: context.read<RecommendationProductsBloc>(),
        child: BlocBuilder<RecommendationProductsBloc,
            RecommendationProductsState>(builder: (context1, state) {
          String mixedSale = '';
          if (isMixedSale!) {
          ...
          return CustomDialog(
              ...
              negativeOnTap: () async {
                ...
                bloc.add(RecommendationProductsEvent.addToCartProductEvent(
  ...
  void showMinMaxQtyConfirmDialog({
    ...
    final RecommendationProductsBloc bloc =
        context.read<RecommendationProductsBloc>();
