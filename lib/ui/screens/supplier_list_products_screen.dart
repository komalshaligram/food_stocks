    ...
    return BlocProvider(
        create: (context) => SupplierListProductsBloc()
          ...
          ..add(SupplierListProductsEvent.getSupplierProductsIdEvent(
              supplierId: args?[AppStrings.supplierIdString] ?? '', search: args?[AppStrings.searchString] ?? ''))
          ..add(SupplierListProductsEvent.getSupplierDeliveryScheduleEvent(context: context))
          ..add(SupplierListProductsEvent.getSupplierAppInfoEvent(context: context))
          ..add(SupplierListProductsEvent.getSupplierProductsListEvent(context: context, searchType: args?[AppStrings.searchType] ?? ''))
...
class SupplierListProductsScreenWidget extends StatelessWidget {
              ...
              child: CommonAppBar(
                ...
                trailingWidget: Row(mainAxisSize: MainAxisSize.min, children: [
                  GestureDetector(
                      ...
                      child: Icon(Icons.tune, color: state.sortOption == ProductSortOption.none ? null : AppColors.mainColor)),
                      ...
                      Column(children: [
                        ...
                        Expanded(
                          child: SmartRefresher(
                            physics: (!state.isShimmering && state.productList.isEmpty)
                            ...
                            onRefresh: () {
                              ...
                              context
                                  ...
                                  .add(SupplierListProductsEvent.getSupplierDeliveryScheduleEvent(context: context));
                              context.read<SupplierListProductsBloc>().add(SupplierListProductsEvent.getSupplierAppInfoEvent(context: context));
                            },
                            child: !state.isShimmering && state.productList.isEmpty
                                ? Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
                                    SupplierProductsInfoBar(
                                        ...
                                        supplierInfo: state.supplierAppInfo,
                    ...
                    onNotification: (notification) {
                      ...
                      if (notification.metrics.pixels > (notification.metrics.maxScrollExtent - 400)) {
                        if (!state.isBottomOfProducts) {
                          context
                              .read<SupplierListProductsBloc>()
                              .add(SupplierListProductsEvent.getSupplierProductsListEvent(context: context, searchType: state.searchType));
                        } else {
                          return false;
                        }
                      }
  ...
  Widget categoryDataWidget(BuildContext context, SupplierListProductsState state) => AnimatedCrossFade(
      ...
      secondChild: Column(children: [
        ...
        if (state.selectedCategoryId.isNotEmpty && state.subCategoryList.isNotEmpty) subCategoryRowWidget(context, state),
      ]),
  ...
  void _onCategoryTap(BuildContext context, String categoryId) {
    context.read<SupplierListProductsBloc>().add(SupplierListProductsEvent.selectCategoryEvent(context: context, categoryId: categoryId));
  }

  Widget subCategoryRowWidget(BuildContext context, SupplierListProductsState state) {
    return SizedBox(
      width: getScreenWidth(context),
      height: 44,
      child: ListView.builder(
  ...
  void _onSubCategoryTap(BuildContext context, String subCategoryId) {
    context.read<SupplierListProductsBloc>().add(SupplierListProductsEvent.selectSubCategoryEvent(context: context, subCategoryId: subCategoryId));
  }
  ...
  void _onBrandTap(BuildContext context, String brandId) {
    context.read<SupplierListProductsBloc>().add(SupplierListProductsEvent.selectBrandEvent(context: context, brandId: brandId));
  }

  Widget buildSupplierListDataItem({String supplierLogo = '', String? label, required void Function() onTap, required bool isSelected}) {
    return TweenAnimationBuilder<double>(
      ...
      child: ClipRRect(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
              borderRadius: const BorderRadius.all(Radius.circular(AppConstants.radius_10)),
              gradient: (label != null && isSelected) ? AppColors.appMainGradientColor : null,
            ...
            child: Padding(
              ...
              child: label != null
                  ...
                  : supplierLogo.isNotEmpty
                      ...
                      : Image.asset(AppImagePath.imageNotAvailable5, fit: BoxFit.contain, width: double.infinity, height: double.infinity),
            ),
          ),
        ),
      ),
  ...
  void handleIncrease(BuildContext context, SupplierListProductsState state, int index) {
    ...
    if (!isMixedSale && minQty <= quantity + 1) {
      context.read<SupplierListProductsBloc>().add(SupplierListProductsEvent.increaseListQuantityOfProduct(
          context: context, productListIndex: 1, productStockUpdateIndex: index, productSupplierIds: state.productList[index].supplierId.toString()));
      ...
      showMinMaxQtyConfirmDialog(
          context: context, productId: state.productList[index].id.toString(), index: index, productListIndex: 1, isIncrease: true);
    }
  }
  ...
  void handleDecrease(BuildContext context, SupplierListProductsState state, int index) {
    ...
    if (!isMixedSale && minQty <= quantity - 1) {
      ...
      showMinMaxQtyConfirmDialog(
          context: context, productId: state.productList[index].id.toString(), index: index, productListIndex: 1, isIncrease: false);
    }
  }
  ...
  List<int> sortedProductIndices(SupplierListProductsState state) {
    final int count = state.productList.length < state.productStockList[1].length ? state.productList.length : state.productStockList[1].length;
    final List<int> indices = List<int>.generate(count, (i) => i);
    switch (state.sortOption) {
      case ProductSortOption.aToZ:
        indices
            .sort((a, b) => (state.productList[a].productName ?? '').toLowerCase().compareTo((state.productList[b].productName ?? '').toLowerCase()));
        break;
      case ProductSortOption.priceLowToHigh:
        indices.sort((a, b) => _productPrice(state.productList[a]).compareTo(_productPrice(state.productList[b])));
        break;
      case ProductSortOption.priceHighToLow:
        indices.sort((a, b) => _productPrice(state.productList[b]).compareTo(_productPrice(state.productList[a])));
        break;
  ...
  double _productPrice(ProductData product) => double.tryParse(product.productPrice?.toString() ?? '') ?? 0;

  void showSortOptionsBottomSheet(BuildContext context, SupplierListProductsState state) {
          ...
          return SafeArea(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Padding(
                  ...
                  child: Text(l10n.sort_products, style: AppStyles.rkBoldTextStyle(size: AppConstants.font_20, color: AppColors.blackColor))),
              ...
              ...options.map((opt) {
                ...
                return ListTile(
                    title: Text(opt.value,
                        style:
                            AppStyles.rkRegularTextStyle(size: AppConstants.font_14, color: selected ? AppColors.mainColor : AppColors.blackColor)),
                    trailing: selected ? Icon(Icons.check, color: AppColors.mainColor) : null,
                    onTap: () {
                      Navigator.pop(sheetContext);
                      bloc.add(SupplierListProductsEvent.applySortOptionEvent(context: context, option: opt.key));
                    });
              }),
  ...
  Widget searchWidget(BuildContext context, SupplierListProductsBloc bloc, SupplierListProductsState state) => CommonSearchWidget(
      ...
      onScanTap: () async {
        ...
        if (scanResult != '-1') {
          if (!state.isGuestUser) {
            showProductDetails(
                productListIndex: 0, context: context, productId: scanResult, isBarcode: true, productStock: '1', isSaleOn: state.isSaleOn);
          } else {
            Navigator.pushNamed(context, RouteDefine.connectScreen.name);
          }
        }
                                      ...
                                      CommonProductDetailsWidget(
                                          ...
                                          addToOrderTap: () {
                                            ...
                                            if (!isMixedSale &&
                                              ...
                                              context
                                                  .read<SupplierListProductsBloc>()
                                                  .add(SupplierListProductsEvent.addToCartProductEvent(context: context1, productId: productId));
                                            } else {
                                          ...
                                          onQuantityChanged: (quantity) {
                                            context
                                                .read<SupplierListProductsBloc>()
                                                .add(SupplierListProductsEvent.updateQuantityOfProduct(context: context1, quantity: quantity));
                                          },
                                          onQuantityIncreaseTap: () {
                                            context
                                                .read<SupplierListProductsBloc>()
                                                .add(SupplierListProductsEvent.increaseQuantityOfProduct(context: context1));
                                          },
                                          onQuantityDecreaseTap: () {
                                            if (state.productStockList[state.productListIndex][state.productStockUpdateIndex].quantity > 1) {
                                              context
                                                  .read<SupplierListProductsBloc>()
                                                  .add(SupplierListProductsEvent.decreaseQuantityOfProduct(context: context1));
                                            }
                                          },
                                          onCloseTap: () async {
                                            final bloc = context.read<SupplierListProductsBloc>();
        ...
        child: ListView.builder(
            ...
            itemBuilder: (context2, i) {
              ...
              return CommonProductSaleItemWidget(
                  ...
                  isGuestUser: context.read<SupplierListProductsBloc>().state.isGuestUser,
                  onGuestLoginRequired: () => Navigator.pushNamed(context, RouteDefine.connectScreen.name),
                  height: AppConstants.salesProductItemHeight,
                  ...
                  onQuantityIncreaseTap: () {
                    ...
                    if (!(relatedProduct.sale?.isMixedSale ?? false) &&
                        int.parse(relatedProduct.sale?.saleMinQuantity ?? '0') <= productStockList[2][relatedStockIdx].quantity + 1) {
                      context.read<SupplierListProductsBloc>().add(SupplierListProductsEvent.increaseListQuantityOfProduct(
                          context: context,
                          productListIndex: 2,
                          productStockUpdateIndex: relatedStockIdx,
                      ...
                      showMinMaxQtyConfirmDialog(
                          context: context, productId: relatedProduct.id.toString(), index: relatedStockIdx, productListIndex: 2, isIncrease: true);
                    }
                  },
                  onQuantityDecreaseTap: () {
                    if (!hasRelatedStock || relatedStockIdx == -1) return;
  ...
  void showMinMaxQtyConfirmDialog(
      ...
      int? initialQuantity}) {
    final stockState = context.read<SupplierListProductsBloc>().state;
