...
/// Opens the promotion bottom sheet for a min-quantity sale.
///
/// The tapped product's [productId] is enough: the sheet loads the whole sale
/// group (the product itself for a plain min-quantity sale, or every
/// participating product for a mixed sale) together with the client's current
/// cart quantities, lets the user set the amount of each with a stepper while a
...
/// Mutable per-row model backing a stepper inside the sheet.
class _PromoItem {
  _PromoItem({
    required this.id,
    required this.name,
  ...
  /// Empty until the product already has a cart line (needed for update/remove).
  String cartProductId;

  /// Quantity already in the cart when the sheet opened.
  final int originalQty;

  /// Quantity the user currently wants (drives the steppers + counter).
  int desiredQty;
...
class _SalePromotionSheetState extends State<SalePromotionSheet> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final res = await dio.post(
        AppUrlEndPoints.getSaleParticipatingProductsUrl,
      ...
      // Leaves _items empty → the sheet shows its empty state.
    } finally {
  ...
  Future<void> _commit() async {
    ...
    try {
      // New lines are inserted together; existing lines are updated/removed.
  ...
  Future<bool> _insertLines(DioClient dio, List<insert.Product> products) async {
    ...
    final res = await dio.post(
      '${AppUrlEndPoints.insertProductInCartUrl}$_cartId',
      data: req,
    );
  ...
  Future<bool> _updateLine(DioClient dio, _PromoItem item) async {
    ...
    final res = await dio.post(
      '${AppUrlEndPoints.updateCartProductUrl}$_cartId',
      data: request,
    );
    return res is Map && res['status'] == AppConstants.code_201;
  }
    ...
    return Directionality(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: const BorderRadius.vertical(top: Radius.circular(AppConstants.radius_25)),
        ),
        child: Column(
  ...
  Widget _buildHeader() => Padding(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                _isMixed ? l10n.mixedSale : l10n.promo_sheet_reach_min_title,
            ...
            InkWell(
              ...
              child: Padding(
                ...
                child: Icon(Icons.close, color: AppColors.greyColor, size: 22),
              ),
            ),
          ],
        ),
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Text(
                l10n.promo_sheet_of_packages(_selectedTotal, _min),
          ...
          6.height,
          Text(
            _minReached ? l10n.promo_sheet_min_reached : l10n.promo_sheet_remaining(remaining),
  ...
  Widget _buildRow(_PromoItem item) {
    ...
    printData("check here price ${'₪${item.salePrice * item.desiredQty * int.parse(item.numberOfUnit)}'}");
            ...
            child: Column(
              ...
              children: [
                ...
                if (item.numberOfUnit.isNotEmpty && item.numberOfUnit != '0') ...[
                  2.height,
                  Text(
                    '${item.numberOfUnit} ${l10n.unit_in_box}',
                ...
                Row(
                  ...
                  children: [
                    Text(
                      ...
                      boxPriceTotal,
                      style: AppStyles.rkBoldTextStyle(size: AppConstants.font_15, color: AppColors.saleRedColor),
                    ),
                    if (hasDiscount) ...[
                      6.width,
                      Text(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          _stepperButton(
            icon: Icons.remove,
            enabled: canDecrease && !_committing,
      ...
      GestureDetector(
        ...
        child: Container(
          ...
          child: Icon(
            icon,
            size: 18,
            color: filled ? AppColors.whiteColor : (enabled ? AppColors.blackColor : AppColors.borderColor),
          ),
        ),
      );
        ...
        child: Column(
          ...
          children: [
            ...
            GestureDetector(
              ...
              child: Text(
                l10n.closeText,
                style: AppStyles.rkBoldTextStyle(size: AppConstants.font_15, color: AppColors.mainColor),
              ),
            ),
          ],
        ),
