...
class SupplierProductsInfoBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      margin: margin ?? const EdgeInsets.fromLTRB(AppConstants.padding_10, AppConstants.padding_2, AppConstants.padding_10, 0),
      decoration: BoxDecoration(
          color: AppColors.whiteColor,
          borderRadius: BorderRadius.circular(AppConstants.radius_10),
          border: Border.all(color: AppColors.lightBorderColor),
          boxShadow: [BoxShadow(color: AppColors.shadowColor.withValues(alpha: 0.06), blurRadius: 10, offset: const Offset(0, 3))]),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(AppConstants.radius_10),
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          IntrinsicHeight(
            child: Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              Expanded(flex: _showDeliverySection ? 1 : 2, child: _MinimumOrderSection(label: l10n.minimum_order, amount: minimumOrder)),
              if (_showDeliverySection) ...[
                VerticalDivider(width: 1, thickness: 1, color: AppColors.lightBorderColor),
                Expanded(
                  child: _DeliveryScheduleSection(
                      ...
                      onTap: deliveryCityName?.isNotEmpty != true || deliveryDays.isEmpty
                          ? null
                          : () => SupplierDeliveryScheduleWidget.showSheet(context, cityName: deliveryCityName ?? '', deliveryDays: deliveryDays)),
                )
              ]
            ]),
          ),
          if (_showInfoSection) ...[
            Divider(height: 1, thickness: 1, color: AppColors.lightBorderColor),
...
class _MinimumOrderSection extends StatelessWidget {
  const _MinimumOrderSection({required this.label, required this.amount});
...
class _SupplierInfoSection extends StatelessWidget {
  const _SupplierInfoSection({required this.label, required this.subLabel, required this.onTap});
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: InkWell(
        onTap: onTap,
        child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: AppConstants.padding_10, vertical: AppConstants.padding_8),
          child: Row(children: [
            Container(
                width: 32,
                height: 32,
                decoration: BoxDecoration(gradient: AppColors.appMainGradientColor, shape: BoxShape.circle),
                alignment: Alignment.center,
                child: Icon(Icons.info_outline, color: AppColors.whiteColor, size: 18)),
            10.width,
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                Text(label,
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: AppStyles.rkBoldTextStyle(size: AppConstants.font_13, color: AppColors.mainColor)),
                2.height,
                Text(subLabel,
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
            ...
            4.width,
            Icon(Icons.chevron_right, color: AppColors.mainColor, size: 22)
          ]),
        ),
      ),
    );
...
class _DeliveryScheduleSection extends StatelessWidget {
  const _DeliveryScheduleSection(
      {required this.isLoading, required this.cityName, required this.deliveryDays, required this.label, required this.onTap});
  ...
  Widget build(BuildContext context) {
    if (isLoading) {
      return Container(
          ...
          child: SizedBox(width: 22, height: 22, child: CupertinoActivityIndicator(color: AppColors.blueColor)));
    }
        ...
        child: Padding(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            Container(
                ...
                decoration: BoxDecoration(gradient: AppColors.appMainGradientColor, shape: BoxShape.circle),
                alignment: Alignment.center,
                child: Transform.flip(flipX: true, child: Icon(Icons.local_shipping_outlined, color: AppColors.whiteColor, size: 18))),
            10.width,
            Flexible(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                Text(label,
