...
class CustomerServiceContactButton extends StatelessWidget {
  ...
  const CustomerServiceContactButton(
      {super.key, required this.customerServicePhone, required this.customerServiceWhatsApp, this.isGuestUser = false, this.onGuestLoginRequired});
      ...
      child: InkWell(
        ...
        onTap: () {
          ...
          showCustomerServiceBottomSheet(
              context: context, customerServicePhone: customerServicePhone, customerServiceWhatsApp: customerServiceWhatsApp);
        },
        child: Container(
          height: 60,
          width: 60,
          decoration: BoxDecoration(
              ...
              boxShadow: [
                BoxShadow(color: AppColors.shadowColor.withValues(alpha: 0.18), blurRadius: AppConstants.blur_10, offset: const Offset(0, 4))
              ],
              borderRadius: BorderRadius.circular(AppConstants.radius_100),
              border: Border.all(color: AppColors.lightBorderColor.withValues(alpha: 0.8))),
          child: Container(
              margin: const EdgeInsets.all(4),
              decoration: BoxDecoration(gradient: AppColors.appMainGradientColor, borderRadius: BorderRadius.circular(AppConstants.radius_100)),
...
Future<void> showCustomerServiceBottomSheet(
    {required BuildContext context, required String customerServicePhone, required String customerServiceWhatsApp}) {
  ...
  return showModalBottomSheet<void>(
      ...
      builder: (sheetContext) {
        return Padding(
          ...
          child: Container(
            decoration: BoxDecoration(color: AppColors.whiteColor, borderRadius: const BorderRadius.vertical(top: Radius.circular(24))),
            child: SafeArea(
              top: false,
              child: Padding(
                padding:
                    const EdgeInsets.fromLTRB(AppConstants.padding_20, AppConstants.padding_10, AppConstants.padding_20, AppConstants.padding_20),
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  Container(
                    width: 44,
                    height: 4,
                    decoration: BoxDecoration(
                        color: AppColors.lightGreyColor.withValues(alpha: 0.55), borderRadius: BorderRadius.circular(AppConstants.radius_100)),
                  ),
                  16.height,
                  Text(l10n.customer_service_sheet_title,
                      ...
                      onTap: () async {
                        if (customerServiceWhatsApp.trim().isEmpty) {
                          CustomSnackBar.showSnackBar(
                              context: sheetContext, title: l10n.customer_service_whatsapp_unavailable, type: SnackBarType.failure);
                          return;
                        }
                  ...
                  _CustomerServiceActionTile(
                      ...
                      onTap: () async {
                        if (customerServicePhone.trim().isEmpty) {
                          CustomSnackBar.showSnackBar(
                              context: sheetContext, title: l10n.customer_service_phone_unavailable, type: SnackBarType.failure);
                          return;
                        }
...
class _CustomerServiceActionTile extends StatelessWidget {
  ...
  const _CustomerServiceActionTile(
      {required this.title, required this.subtitle, required this.iconWidget, required this.iconBackgroundColor, required this.onTap});
          ...
          child: Row(children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                4.height,
                Text(subtitle, style: AppStyles.rkRegularTextStyle(size: AppConstants.smallFont, color: AppColors.greyColor)),
              ]),
            ),
