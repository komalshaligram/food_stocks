...
debugPrint('RES:${response.toString()}');
        ...
        if (e.response?.statusCode == AppConstants.code_401 && path != AppUrls.refreshTokenUrl) {
          tokenExpirationWork(
              path, data, preferencesHelper,AppStrings.post_method, queryParameters ?? {});
        } else if (path == AppUrls.refreshTokenUrl &&
            e.response?.statusCode == AppConstants.code_401) {
    ...
    try {
      ...
      if (Platform.isAndroid) {
        try {
          ...
          if (e.response?.statusCode == AppConstants.code_401 &&
              path != AppUrls.refreshTokenUrl) {
            tokenExpirationWork(
                path, null, preferencesHelper, AppStrings.get_method, query ?? {});
          } else if (path == AppUrls.refreshTokenUrl &&
              e.response?.statusCode == AppConstants.code_401) {
      ...
      if(Platform.isAndroid){
        ...
        try {
          ...
          if (e.response?.statusCode == AppConstants.code_401 &&
              path != AppUrls.refreshTokenUrl) {
            tokenExpirationWork(
                path, data, preferencesHelper, AppStrings.put_method, query ?? {});
          } else if (path == AppUrls.refreshTokenUrl &&
              e.response?.statusCode == AppConstants.code_401) {
            manageRefreshTokenWork(preferencesHelper, query ?? {});
          } else {
