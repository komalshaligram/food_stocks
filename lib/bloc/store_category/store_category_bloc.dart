    ...
    on<StoreCategoryEvent>((event, emit) async {
      ...
      if (event is _changeCategoryExpansionEvent) {
        ...
        if (event.isOpened != null) {
          emit(state.copyWith(
              isCategoryExpand: event.isOpened ?? false,
        ...
        emit(state.copyWith(
            isSubCategory: event.isSubCategory, subCategoryId: ''));
        if (categoryId != '') {
          ...
          emit(
              state.copyWith(planogramPageNum: 0, isBottomOfPlanoGrams: false));
          add(StoreCategoryEvent.changeCategoryDetailsEvent(
            context: event.context,
            isSubCategory: '',
            categoryName: state.categoryName,
        ...
        if (event.isSubCategory == '') {
          // Load the products immediately, planograms in parallel after.
        ...
        if (event.subCategoryId != state.subCategoryId) {
          emit(state.copyWith(
              ...
              isBottomOfPlanoGrams: false,
              isBottomOfProducts: false));
          // Products first (fast path), planograms in parallel after.
          add(StoreCategoryEvent.getPlanogramAllProductEvent(
              context: event.context));
          add(StoreCategoryEvent.getPlanoGramProductsEvent(
              context: event.context));
        ...
        try {
          ...
          final res = await DioClient(event.context)
              .post(AppUrlEndPoints.getSubCategoriesUrl,
          ...
          if (response.status == AppConstants.code_200) {
            ...
            emit(state.copyWith(
              ...
              isLoadMore: false,
              bottleDeposit: preferences.getBottleTax(),
              categoryName: response.data!.subCategories!.isNotEmpty
                  ? (response.data?.subCategories?[0].parentCategoryName ?? '')
                  : state.categoryName,
            ));
            emit(state.copyWith(
        ...
        if (state.isBottomOfPlanoGrams) {
          if (state.isSubCategory) {
            add(StoreCategoryEvent.getSubCategoryListEvent(
                context: event.context));
          }
          // Products are dispatched explicitly (in parallel) so they no longer
          // wait for the planogram round-trip.
          return;
        }
        try {
          emit(state.copyWith(
          ...
          PlanogramReqModel planogramReqModel =
          ...
          // Drop empty strings too: an empty subCategoryId (the "All" case) makes
          // the planogram endpoint 500 on `new ObjectId('')`, which would block
          // the product fetch below. With categoryId only it returns 200.
          ...
          if (response.status == AppConstants.code_200) {
            ...
            for (int i = 0; i < (response.data?.length ?? 0); i++) {
              stockList.addAll(response.data![i].planogramproducts
                  ?.map((product) => ProductStockModel(
                productId: product.id ?? '',
                maxQty: (product.sale?.isSale ?? false)
                    ? int.parse(
                    product.sale?.saleMaxQuantity.toString() ??
                        '0')
                    : 0,
        ...
        emit(state.copyWith(
            ...
            isBottomOfProducts: false,
            isBottomOfPlanoGrams: false));
        // Products first (fast), planograms in parallel — products no longer
        // chain off the planogram fetch.
        add(StoreCategoryEvent.getPlanogramAllProductEvent(
            context: event.context));
        add(StoreCategoryEvent.getPlanoGramProductsEvent(
            context: event.context));
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            if (response.product!.isNotEmpty) {
              ...
              if (response.product!.isNotEmpty) {
                add(StoreCategoryEvent.relatedProductsEvent(
                    context: event.context,
                    productId: response.product?.first.id ?? ''));
              }
        ...
        List<List<ProductStockModel>> productStockList =
        List.from(state.productStockList, growable: true);
        List<ProductStockModel> stockList = [];
        ...
        if (state.isBottomOfProducts) {
          ...
          for (var product in planogramProductList) {
            stockList.add(ProductStockModel(
              maxQty: (product.product.sale?.isSale ?? false)
                  ? int.parse(product.product.sale?.saleMaxQuantity ?? '0')
                  : 0,
        ...
        try {
          ...
          // "All" (empty subCategoryId) → send categoryId so the backend returns
          // every product in the category; otherwise filter by the sub-category.
          // (The endpoint 500s on an empty subCategoryId — never send it.)
          final bool isAllProducts = state.subCategoryId.isEmpty;
          GetSubCategoriesProductReqModel getSubCategoriesProductReqModel =
          GetSubCategoriesProductReqModel(
          ...
          final Map<String, dynamic> productReqBody =
          getSubCategoriesProductReqModel.toJson()
            ..removeWhere((key, value) => value == null);

          final res = await DioClient(event.context).post(
              AppUrlEndPoints.getSubCategoryProductsUrl,
              data: productReqBody);
          GetPlanogramProductModel response =
          GetPlanogramProductModel.fromJson(res);
          if (response.status == AppConstants.code_200) {
            final cartMap = await fetchCartQuantities(event.context);
            ...
            emit(state.copyWith(
                isLoadMore: false, isPlanogramProductShimmering: false));
          }
        } on ServerException {
          emit(state.copyWith(
              isLoadMore: false, isPlanogramProductShimmering: false));
