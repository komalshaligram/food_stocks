...
import '../../data/model/req_model/insert_cart_req_model/insert_cart_req_model.dart' as insert_cart_model;
      ...
      if (event is _changeCategoryExpansionEvent) {
        ...
        if (categoryId != '') {
          add(StoreCategoryEvent.changeCategoryDetailsEvent(
              context: event.context, isSubCategory: 'true', categoryName: state.categoryName, categoryId: parentCategoryId));
        } else {
          emit(state.copyWith(planogramPageNum: 0, isBottomOfPlanoGrams: false));
          add(StoreCategoryEvent.changeCategoryDetailsEvent(
              context: event.context, isSubCategory: '', categoryName: state.categoryName, categoryId: state.categoryId));
        }
        ...
        if (event.subCategoryId != state.subCategoryId) {
          emit(state.copyWith(
              ...
              subPlanoGramsList: [],
              productStockList: [state.productStockList[0], state.productStockList[1], [], []],
              bottleDeposit: preferences.getBottleTax(),
              planogramProductList: [],
              subProductPageNum: 0,
        ...
        try {
          emit(state.copyWith(
              isSubCategoryShimmering: state.subCategoryPageNum == 0 ? true : false, isLoadMore: state.subCategoryPageNum == 0 ? false : true));
          final res = await DioClient(event.context).post(AppUrlEndPoints.getSubCategoriesUrl,
              data: ProductSubcategoriesReqModel(
                      parentCategoryId: state.categoryId, pageNum: state.subCategoryPageNum + 1, pageLimit: AppConstants.productSubCategoryPageLimit)
                  .toJson());
          ...
          if (response.status == AppConstants.code_200) {
            ...
            emit(state.copyWith(
                ...
                bottleDeposit: preferences.getBottleTax(),
                categoryName:
                    response.data!.subCategories!.isNotEmpty ? (response.data?.subCategories?[0].parentCategoryName ?? '') : state.categoryName));
        ...
        try {
          emit(state.copyWith(
              isPlanogramShimmering: state.planogramPageNum == 0 ? true : false, isLoadMore: state.planogramPageNum == 0 ? false : true));
          ...
          req.removeWhere((key, value) => value == null || value == '');
            ...
            if (response.product!.isNotEmpty) {
              ...
              if (event.isBarcode) {
                ...
                productStockUpdateIndex =
                    state.productStockList[planoGramIndex].indexWhere((productStock) => productStock.productId == event.productId);
              }
              ...
              supplierList.addAll(response.product?.first.supplierSales
                      ?.map((supplier) => ProductSupplierModel(
                            ...
                            quantity: _productQuantity,
                            maxQty:
                                (response.product?.first.sale?.isSale ?? false) ? int.parse(response.product?.first.sale?.saleMaxQuantity ?? '0') : 0,
                            stock: supplier.productStock.toString(),
                            selectedIndex: (supplier.supplierId ?? '') ==
                                    state.productStockList[planoGramIndex][productStockUpdateIndex].productSupplierIds
                                ? !supplier.saleProduct!.contains(supplier.saleProduct?.firstWhere(
                                    ...
                                    : supplier.saleProduct?.indexOf(
                                          supplier.saleProduct?.firstWhere(
                                                  (sale) =>
                                                      sale.saleId == state.productStockList[planoGramIndex][productStockUpdateIndex].productSaleId,
                                                  orElse: () => const SaleProduct(
                                                      isSale: false,
                                                      saleDescription: '',
                                                      saleFromDate: '',
        ...
        if (state.productStockUpdateIndex != -1) {
          if (productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].quantity <
              double.parse(productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].stock.toString())) {
            ...
            if (productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].maxQty != 0) {
              if (productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].quantity >=
                  productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].maxQty) {
                CustomSnackBar.showSnackBar(
                    context: event.context, title: AppLocalizations.of(event.context)!.not_add_more_than_max_qty, type: SnackBarType.failure);
                return;
              }
            }
            productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex] = productStockList[state.planoGramUpdateIndex]
                    [state.productStockUpdateIndex]
                .copyWith(quantity: productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].quantity + 1);
            emit(state.copyWith(productStockList: []));
            emit(state.copyWith(productStockList: productStockList));
          } else {
            CustomSnackBar.showSnackBar(
        ...
        if (state.productStockUpdateIndex != -1) {
          ...
          if (newQuantity <= double.parse(productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].stock.toString())) {
            productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex] =
                productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].copyWith(quantity: newQuantity);
            emit(state.copyWith(productStockList: []));
            emit(state.copyWith(productStockList: productStockList));
          } else {
            productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex] = productStockList[state.planoGramUpdateIndex]
        ...
        List<List<ProductStockModel>> productStockList = state.productStockList.toList(growable: true);
        productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex] =
            productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].copyWith(
        ...
        supplierList[event.supplierIndex] = supplierList[event.supplierIndex]
            .copyWith(selectedIndex: supplierList[event.supplierIndex].selectedIndex == event.supplierSaleIndex ? -1 : event.supplierSaleIndex);
        ...
        if (state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].maxQty > 0) {
          if (state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].quantity >
              state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].maxQty) {
            CustomSnackBar.showSnackBar(
                context: event.context, title: AppLocalizations.of(event.context)!.not_add_more_than_max_qty, type: SnackBarType.failure);
            return;
          }
        ...
        if (_isProductInCart) {
          try {
            ...
            UpdateCartReqModel request = UpdateCartReqModel(
                productId: state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].productId == ''
                    ? event.productId
                    : state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].productId,
                supplierId: state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].productSupplierIds,
                saleId: state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].productSaleId == ''
                    ? null
                    : state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].productSaleId,
          ...
          try {
            emit(state.copyWith(isLoading: true));
            insert_cart_model.InsertCartReqModel insertCartReqModel = insert_cart_model.InsertCartReqModel(products: [
              insert_cart_model.Product(
                  ...
                  quantity: state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].quantity,
                  supplierId: state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].productSupplierIds,
                  note: state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].note.isEmpty
                      ? null
                      : state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].note,
                  saleId: state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].productSaleId.isEmpty
                      ? null
                      : state.productStockList[state.planoGramUpdateIndex][state.productStockUpdateIndex].productSaleId)
            ]);
        ...
        try {
          GlobalSearchReqModel globalSearchReqModel =
              GlobalSearchReqModel(search: state.search, sortField: AppStrings.sortFieldString, sortOrder: AppStrings.sortOrderString);
          ...
          if (state.searchController.text.isNotEmpty && state.searchController.text != requestedSearch) {
            return;
          }
        ...
        try {
          ...
          final bool isAllProducts = state.subCategoryId.isEmpty;
          GetSubCategoriesProductReqModel getSubCategoriesProductReqModel = GetSubCategoriesProductReqModel(
              categoryId: isAllProducts ? state.categoryId : null,
              subCategoryId: isAllProducts ? null : state.subCategoryId,
              pageNum: state.subProductPageNum + 1,
              pageLimit: AppConstants.orderPageLimit);
          final Map<String, dynamic> productReqBody = getSubCategoriesProductReqModel.toJson()..removeWhere((key, value) => value == null);

          final res = await DioClient(event.context).post(AppUrlEndPoints.getSubCategoryProductsUrl, data: productReqBody);
        ...
        if (response.status == AppConstants.code_200) {
          ...
          emit(state.copyWith(
              relatedProductList: response.data ?? [], isProductLoading: false, isRelatedShimmering: false, productStockList: productStockList));
        } else {
        ...
        if (state.productStockList[event.productListIndex][event.productStockUpdateIndex].quantity == 0) {
          ...
          if (_isProductInCart) {
            ...
            try {
              insert_cart_model.InsertCartReqModel insertCartReqModel = insert_cart_model.InsertCartReqModel(products: [
                insert_cart_model.Product(
                    productId: state.productStockList[event.productListIndex][event.productStockUpdateIndex].productId == ''
                        ? event.productId
                        : state.productStockList[event.productListIndex][event.productStockUpdateIndex].productId,
                    quantity: state.productStockList[event.productListIndex][event.productStockUpdateIndex].quantity,
                    supplierId: event.productSupplierIds,
        ...
        final productStockList = state.productStockList
            .map((sub) => sub.map((item) {
                  ...
                  return item.copyWith(quantity: cartMap[pid] ?? 0, cartProductId: '', totalPrice: 0.0);
                }).toList())
            .toList();
        emit(state.copyWith(productStockList: productStockList));
      }
