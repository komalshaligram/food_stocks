  ...
  CompanyProductsBloc() : super(CompanyProductsState.initial()) {
    on<CompanyProductsEvent>((event, emit) async {
      ...
      if (event is _getCompanyProductsIdEvent) {
        ...
        try {
          emit(state.copyWith(isRefreshingProduct: true, isShimmering: state.pageNum == 0 ? true : false, isLoadMore: state.pageNum == 0 ? false : true));
          ...
          if (response.status == AppConstants.code_200) {
            List<CompanyData> productList = state.productList.toList(growable: true);
            List<List<ProductStockModel>> productStockList = state.productStockList.toList(growable: true);
            List<ProductStockModel> stockList = [];
            productList.addAll(response.data ?? []);
            stockList.addAll(
              response.data?.map((product) => ProductStockModel(productId: product.productId ?? '', maxQty: (product.product?.sale?.isSale ?? false) ? int.parse(product.product?.sale?.saleMaxQuantity ?? '0') : -1, stock: product.product?.productStock.toString() ?? '0')) ?? [],
            );
            ...
            if (response.product!.isNotEmpty) {
              ...
              if (supplierList.isNotEmpty) {
                ...
                if (!isSupplierSelected || state.productListIndex == 0) {
                  ...
                  add(CompanyProductsEvent.supplierSelectionEvent(supplierIndex: supplierIndex, context: event.context, supplierSaleIndex: supplierSaleIndex));
                }
              }
            } else {
