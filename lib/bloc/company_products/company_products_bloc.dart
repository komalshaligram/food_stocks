    ...
    on<CompanyProductsEvent>((event, emit) async {
      ...
      if (event is _getCompanyProductsIdEvent) {
        ...
        List<ProductData> productList =
            List.from(state.productList, growable: true);
        ...
        try {
          ...
          CompanyProductsReqModel request = CompanyProductsReqModel(
            ...
            categoryId: state.selectedCategoryId.isEmpty
                ? null
                : state.selectedCategoryId,
            subCategoryId: state.selectedSubCategoryId.isEmpty
                ? null
          ...
          final res = await DioClient(event.context)
              .post(AppUrlEndPoints.getBrandCategoryProductsUrl, data: req);
          SupplierListProductsResponseModel response =
              SupplierListProductsResponseModel.fromJson(res);
          if (response.status == AppConstants.code_200) {
            ...
            emit(state.copyWith(
              productList: productList,
              productStockList: productStockList,
              categoryList: response.data?.categories ?? state.categoryList,
              // Sub-categories come back only when a category is selected; stable
              // across sub-category selection (computed before that filter).
        ...
        final String targetSubCategoryId =
            isDeselect ? '' : event.subCategoryId;
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            if (response.product!.isNotEmpty) {
              ...
              if (response.product!.isNotEmpty) {
                add(CompanyProductsEvent.relatedProductsEvent(
                    context: event.context,
                    productId: response.product?.first.id ?? ''));
              }
              if (event.isBarcode) {
        ...
        if (event.supplierIndex >= 0) {
          ...
          List<List<ProductStockModel>> productStockList =
              state.productStockList.toList(growable: true);
          productStockList[state.productListIndex]
              [state.productStockUpdateIndex] = productStockList[
                  state.productListIndex][state.productStockUpdateIndex]
              .copyWith(
                  productSupplierIds:
        ...
        preferences.setCompanyGridListView(
            isCompanyProductGrid: !state.isCompanyProductGrid);
