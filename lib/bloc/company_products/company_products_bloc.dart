...
class CompanyProductsBloc extends Bloc<CompanyProductsEvent, CompanyProductsState> {
      ...
      if (event is _getCompanyProductsIdEvent) {
        ...
        try {
          ...
          CompanyProductsReqModel request = CompanyProductsReqModel(
              ...
              categoryId: state.selectedCategoryId.isEmpty ? null : state.selectedCategoryId,
          ...
          final Map<String, dynamic> req = request.toJson()..removeWhere((key, value) => value == null);
          final res = await DioClient(event.context).post(AppUrlEndPoints.getBrandCategoryProductsUrl, data: req);
          SupplierListProductsResponseModel response = SupplierListProductsResponseModel.fromJson(res);
          if (response.status == AppConstants.code_200) {
            ...
            emit(state.copyWith(
                isBottomOfProducts: state.productList.length >= (response.metaData?.totalFilteredCount ?? 0) ? true : false, isProgress: false));
          } else {
