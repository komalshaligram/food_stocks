      ...
      if (event is _getProductCategoriesListEvent) {
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            emit(state.copyWith(productCategoryList: productCategoryList, pageNum: state.pageNum + 1, isLoadMore: false, isShimmering: false));
        ...
        emit(state.copyWith(pageNum: 0, productCategoryList: [], isBottomOfCategories: false));
        add(ProductCategoryEvent.getProductCategoriesListEvent(context: event.context));
      } else if (event is _setSearchNavEvent) {
        emit(state.copyWith(reqSearch: event.reqSearch, isFromStoreCategory: event.isFromStoreCategory));
