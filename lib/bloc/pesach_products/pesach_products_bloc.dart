...
class PesachProductsBloc extends Bloc<PesachProductsEvent, PesachProductsState> {
  ...
  PesachProductsBloc() : super(PesachProductsState.initial()) {
    on<PesachProductsEvent>((event, emit) async {
      ...
      if (event is _getSupplierProductsListEvent) {
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            emit(state.copyWith(
                ...
                isLoadMore: false));
            emit(state.copyWith(
                isBottomOfProducts: productList.length == (response.metaData?.totalFilteredCount ?? 0) ? true : false, isProgress: false));
          } else {
        ...
        emit(state.copyWith(
            pageNum: 0, isProgress: false, productList: [], productStockList: [state.productStockList[0], [], []], isBottomOfProducts: false));
        if (!state.isProgress) {
          add(PesachProductsEvent.getSupplierProductsListEvent(context: event.context, searchType: state.searchType));
        }
