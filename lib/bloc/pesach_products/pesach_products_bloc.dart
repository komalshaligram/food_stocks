      ...
      if (event is _getSupplierProductsListEvent) {
        ...
        try {
          ...
          final res = await DioClient(event.context).post(
              ...
              data: req);
          response = PesachProductsResModel.fromJson(res);
          emit(state.copyWith(searchType: event.searchType.toString()));
          if (response.status == AppConstants.code_200) {
            ...
            emit(state.copyWith(
                isBottomOfProducts: productList.length ==
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            if (response.product!.isNotEmpty) {
              ...
              if (response.product!.isNotEmpty) {
                add(PesachProductsEvent.relatedProductsEvent(
                    context: event.context,
                    productId: response.product?.first.id ?? ''));
              }
        ...
        if (state
          ...
          if (state
            ...
            CustomSnackBar.showSnackBar(
              context: event.context,
              title:
                  AppLocalizations.of(event.context)!.not_add_more_than_max_qty,
              type: SnackBarType.failure,
            );
            return;
          }
