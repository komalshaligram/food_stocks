    ...
    on<BrandsPermissionEvent>((event, emit) async {
      if (event is _getPermissionList) {
        try {
          ...
          final res = await DioClient(event.context).get(path: '${AppUrlEndPoints.getBrandPermissionUrl}${event.subUserId}');
          ...
          if (response.status == AppConstants.code_200) {
            ...
            response.data?.forEach((element) {
              brandPermissionList.add(
                  PermissionModel(title: element.brand?.brandName ?? '', isEnable: element.isAllowed ?? false, brandId: element.brand?.id ?? ''));
            });
      ...
      if (event is _switchButtonEvent) {
        ...
        if (event.index == -1) {
          ...
          brandPermissionList[event.index].isEnable = !brandPermissionList[event.index].isEnable;
          if (brandPermissionList[event.index].isEnable == false) {
            emit(state.copyWith(isSelectAll: false));
          }
          emit(state.copyWith(brandPermissionList: brandPermissionList, isRefresh: !state.isRefresh));
        }
        ...
        try {
          ...
          for (var element in state.brandPermissionList) {
            updateBrandPermission.add(BrandPermission(brandId: element.brandId, isAllowed: element.isEnable));
          }
