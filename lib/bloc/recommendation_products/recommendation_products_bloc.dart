  ...
  RecommendationProductsBloc() : super(RecommendationProductsState.initial()) {
    on<RecommendationProductsEvent>((event, emit) async {
      ...
      if (event is _getPreferencesDataEvent) {
        emit(state.copyWith(clubAgentId: preferences.getClubAgentId(), isGuestUser: preferences.getGuestUser()));
      }
      ...
      if (event is _getRecommendationProductsEvent) {
        ...
        try {
          ...
          RecommendationProductsReqModel request =
              RecommendationProductsReqModel(pageLimit: AppConstants.recommendationProductPageLimit, pageNum: state.pageNum + 1);

          final res = await DioClient(event.context).post(AppUrlEndPoints.getRecommendationProductsUrl, data: request.toJson());
          ...
          if (response.status == AppConstants.code_200) {
            ...
            emit(state.copyWith(
                ...
                isLoadMore: false));
            emit(state.copyWith(
                isBottomOfProducts: state.recommendationProductsList.length == (response.metaData?.totalFilteredCount ?? 0) ? true : false));
          } else {
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            if (response.product!.isNotEmpty) {
              ...
              emit(state.copyWith(
                  productListIndex: productListIndex, productStockUpdateIndex: productStockUpdateIndex, productStockList: productStockList));
              try {
