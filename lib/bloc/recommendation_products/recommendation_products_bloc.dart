  ...
  RecommendationProductsBloc() : super(RecommendationProductsState.initial()) {
    on<RecommendationProductsEvent>((event, emit) async {
      ...
      if (event is _getRecommendationProductsEvent) {
        emit(state.copyWith(isIncludedVat: preferences.getIsIncludedVat(), isSaleOn: preferences.getShowSale(), isSubUserAddToBasket: preferences.getCanAddToBasket(), cartCount: preferences.getCartCount(), isGridView: preferences.getRecommendationProductGrid(), bottleDeposit: preferences.getBottleTax()));
        if (state.isLoadMore) {
          return;
        }
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            List<RecommendationData> recommendationProductsList = state.recommendationProductsList.toList(growable: true);
            ...
            stockList.addAll(response.data?.map((recommendationProduct) => ProductStockModel(productId: recommendationProduct.id ?? '', maxQty: (recommendationProduct.sale?.isSale ?? false) ? int.parse(recommendationProduct.sale?.saleMaxQuantity ?? '0') : -1, stock: recommendationProduct.productStock.toString())) ?? []);
            productStockList[1].addAll(stockList);
            emit(state.copyWith(recommendationProductsList: recommendationProductsList, productStockList: productStockList, pageNum: state.pageNum + 1, isShimmering: false, isLoadMore: false));
          ...
          if (response.status == AppConstants.code_200) {
            ...
            if (response.product!.isNotEmpty) {
              ...
              if (event.isBarcode) {
                ...
                productStockList[0][0] = productStockList[0][0].copyWith(quantity: _productQuantity, productId: response.product?.first.id ?? '', stock: (response.product?.first.supplierSales?.first.productStock.toString() ?? '0'), maxQty: (response.product?.first.sale?.isSale ?? false) ? int.parse(response.product?.first.sale?.saleMaxQuantity ?? '0') : -1, totalPrice: double.parse(response.product?.first.supplierSales?.first.productPrice.toString() ?? '0'));
              } else {
