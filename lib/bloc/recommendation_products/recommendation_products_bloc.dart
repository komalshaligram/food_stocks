  ...
  RecommendationProductsBloc() : super(RecommendationProductsState.initial()) {
    on<RecommendationProductsEvent>((event, emit) async {
      ...
      if (event is _getRecommendationProductsEvent) {
        ...
        List.from(state.recommendationProductsList, growable: true);
          ...
          for (var product in recommendationProductsList) {
            stockList.add(ProductStockModel(
              maxQty: (product.sale?.isSale ?? false)
                  ? int.tryParse(
                  product.sale?.saleMaxQuantity.toString() ?? '0') ??
                  0
                  : 0,
        ...
        add(RecommendationProductsEvent.getPermissionList(
            context: event.context));
        emit(state.copyWith(
          isShimmering: true,
          pageNum: 0,
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            if (response.product!.isNotEmpty) {
              ...
              if (event.isBarcode) {
                ...
                productStockList[0][0] = productStockList[0][0].copyWith(
                  ...
                  maxQty: (response.product?.first.sale?.isSale ?? false)
                      ...
                      : 0,
                  totalPrice: double.parse(response
                      .product?.first.supplierSales?.first.productPrice
                      .toString() ??
                      '0'),
                );
              } else {
              ...
              if (response.product!.isNotEmpty) {
                add(RecommendationProductsEvent.relatedProductsEvent(
                    context: event.context,
                    productId: response.product?.first.id ?? ''));
              }
              if (event.isBarcode) {
        ...
        preferences.setRecommendationProductGridListView(
            isRecommendationProductGrid: !state.isGridView);
