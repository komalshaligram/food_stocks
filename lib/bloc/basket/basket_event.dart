...
class BasketEvent with _$BasketEvent {
  ...
  const factory BasketEvent.productUpdateEvent(
      ...
      required String saleId}) = _productUpdateEvent;
  ...
  const factory BasketEvent.removeCartProductEvent(
      ...
      required bool isFromDelete}) = _removeCartProductEvent;
  ...
  const factory BasketEvent.payWithBankTransferEvent({required BuildContext context, required bool isFromRemovePopUp, required String id}) =
  ...
  const factory BasketEvent.orderSendEvent(
      ...
      required bool isFromRemovePopUp}) = _orderSendEvent;
  ...
  const factory BasketEvent.supplierSelectionEvent({required int supplierIndex, required BuildContext context, required int supplierSaleIndex}) =
  ...
  const factory BasketEvent.generalSettings({required BuildContext context, required BuildContext dialogContext, required bool isRetryLoading}) =
      _generalSettings;

  const factory BasketEvent.updateMaintenanceEvent({required BuildContext context}) = _updateMaintenanceEvent;

  const factory BasketEvent.getSupplierPaymentTypeEvent({required BuildContext context, required String id, required int index}) =
  ...
  const factory BasketEvent.getProfileDetailsEvent({required BuildContext context}) = _getProfileDetailsEvent;
}
