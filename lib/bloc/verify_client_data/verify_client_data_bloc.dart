    ...
    on<VerifyClientDataEvent>((event, emit) async {
      ...
      if (event is _initEvent) {
        ...
        try {
          ...
          final profileResponse = await DioClient(event.context)
        ...
        try {
          emit(state.copyWith(isImageUploading: true, deliveryLocationImageFile: File(pickedFile.path)));
          final response = await DioClient(event.context).uploadFileProgressWithFormData(
              path: AppUrlEndPoints.fileUploadUrl,
              formData: FormData.fromMap(
                  {'deliveryLocationImage': await MultipartFile.fromFile(pickedFile.path, contentType: MediaType('image', 'jpeg'))}));
          ...
          emit(state.copyWith(
              isImageUploading: false, deliveryLocationImageUrl: uploadModel.filepath ?? '', deliveryLocationImageFile: File(pickedFile.path)));
        } on ServerException {
          emit(state.copyWith(isImageUploading: false, deliveryLocationImageFile: null));
          CustomSnackBar.showSnackBar(
              context: event.context, title: AppLocalizations.of(event.context)!.something_is_wrong_try_again, type: SnackBarType.failure);
        ...
        final addressParts = [state.streetNameController.text.trim(), state.streetNumberController.text.trim(), state.selectCity]
            .where((part) => part.isNotEmpty)
            .join(' ');
        final wazeUrl = state.wazeUrl.isNotEmpty ? state.wazeUrl : 'https://waze.com/ul?q=${Uri.encodeComponent(addressParts)}&navigate=no';
        ...
        final hasDeliveryImage = state.deliveryLocationImageUrl.isNotEmpty || state.deliveryLocationImageFile != null;
        if (state.selectCity.isEmpty) {
          CustomSnackBar.showSnackBar(
              context: event.context, title: AppLocalizations.of(event.context)!.please_enter_city, type: SnackBarType.failure);
          return;
        }
        ...
        if (matchedCities.isEmpty || matchedCities.first.id == null) {
          CustomSnackBar.showSnackBar(
              context: event.context, title: AppLocalizations.of(event.context)!.cities_not_available, type: SnackBarType.failure);
          return;
        }
        if (state.wazeUrl.trim().isEmpty) {
          CustomSnackBar.showSnackBar(
              context: event.context, title: AppLocalizations.of(event.context)!.please_set_waze_location, type: SnackBarType.failure);
          return;
        }
        if (!hasDeliveryImage) {
          CustomSnackBar.showSnackBar(
              context: event.context, title: AppLocalizations.of(event.context)!.please_upload_delivery_location_photo, type: SnackBarType.failure);
          return;
        }
        ...
        try {
          ...
          final res = await DioClient(event.context).post('${AppUrlEndPoints.updateProfileDetailsUrl}/${preferences.getUserId()}', data: reqMap);
