  ...
  ActivityTimeBloc() : super(ActivityTimeState.initial()) {
    on<ActivityTimeEvent>((event, emit) async {
      ...
      res_get.ProfileDetailsResModel response =
          const res_get.ProfileDetailsResModel();
      ...
      if (event is _defaultValueAddInListEvent) {
        ...
        if (state.operationTimeList.isEmpty) {
          temp.add(ActivityTimeModel(
              ...
              monday: [
                const Day(
                    until: AppStrings.timeString, from: AppStrings.timeString)
              ]));
          temp.add(ActivityTimeModel(
      ...
      if (event is _timePickerEvent) {
        ...
        if (state.time.isNotEmpty) {
          ...
          String? openingTime = shortVar[event.timeIndex].from!.isEmpty
              ? '24:59'
              : shortVar[event.timeIndex].from;
          String? closingTime = shortVar[event.timeIndex].until!.isEmpty
              ? '24:59'
              : shortVar[event.timeIndex].until;
          ...
          if (shortVar.length > 1 &&
            ...
            afterOpeningTime = shortVar[event.timeIndex + 1].from!.isEmpty
                ? '24:59'
                : shortVar[event.timeIndex + 1].from;
          ...
          if (selectedTime != AppStrings.timeString) {
            if (event.timeIndex > 0) {
              ...
              if (event.openingIndex == 1) {
                if (closingTime == AppStrings.timeString &&
                  ...
                  shortVar.insert(event.timeIndex,
                      Day(from: selectedTime, until: closingTime));
                ...
                if (openingTime == AppStrings.timeString) {
                  CustomSnackBar.showSnackBar(
                      context: event.context,
                      title: AppLocalizations.of(event.context)!
                          .please_select_opening_time,
                      type: SnackBarType.failure);
                } else if (selectTimeZone.isAfter(start)) {
                  shortVar.removeAt(event.timeIndex);
                  shortVar.insert(event.timeIndex,
                      Day(from: openingTime, until: selectedTime));
                } else {
                  CustomSnackBar.showSnackBar(
                      context: event.context,
                      title: AppLocalizations.of(event.context)!
            ...
            CustomSnackBar.showSnackBar(
                context: event.context,
                title: AppLocalizations.of(event.context)!
                    .please_select_time_grater_then_0,
                type: SnackBarType.failure);
          }
        }
      }
          ...
          if (state.operationTimeList[event.rowIndex].monday.length > 1) {
            ...
            if (state.operationTimeList[event.rowIndex].monday[len - 1].from !=
                ...
                state.operationTimeList[event.rowIndex].monday[len - 1].until !=
                    AppStrings.timeString) {
              temp[event.rowIndex].monday.add(const Day(
                  from: AppStrings.timeString, until: AppStrings.timeString));
              emit(state.copyWith(
                  operationTimeList: temp, isRefresh: !state.isRefresh));
            } else {
              CustomSnackBar.showSnackBar(
                  context: event.context,
                  title: AppLocalizations.of(event.context)!
                      .please_select_previous_shift_time,
                  type: SnackBarType.failure);
            }
          } else {
            if (state.operationTimeList[event.rowIndex].monday[0].from !=
              ...
              CustomSnackBar.showSnackBar(
                  context: event.context,
                  title: AppLocalizations.of(event.context)!
                      .please_select_first_shift_time,
                  type: SnackBarType.failure);
            }
          }
        } else {
      ...
      if (event is _activityTimeApiEvent) {
        ...
        for (int i = 0; i < state.operationTimeList.length; i++) {
          if (state.operationTimeList[i].monday != [] &&
              state.operationTimeList[i].monday.isNotEmpty) {
            ...
            if (sundayList.first.from != AppStrings.timeString ||
              ...
              try {
                final response1 = await DioClient(event.context).post(
                    ...
                    data: reqMap);
                if (response1[AppStrings.statusString] ==
