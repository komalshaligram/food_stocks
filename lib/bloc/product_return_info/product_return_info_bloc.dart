      ...
      if (event is _getArgumentEvent) {
        ...
        if (map.isNotEmpty) {
          if (map['list'] != null) {
            ...
            int radioIndex = tempList.indexWhere((e) => e.text.toLowerCase().trim() == normalizedReason.toLowerCase().trim());
          ...
          if (state.selectedRadioTile != 0) {
            if (state.proofFile.path.isNotEmpty || state.proofFile1.path.isNotEmpty || state.proofFile2.path.isNotEmpty) {
              ...
              CustomSnackBar.showSnackBar(
                  context: event.context, title: AppLocalizations.of(event.context)!.add_one_proof_img, type: SnackBarType.failure);
            }
          } else {
            CustomSnackBar.showSnackBar(
                context: event.context, title: AppLocalizations.of(event.context)!.select_one_option, type: SnackBarType.failure);
          }
        } else {
          ...
          try {
            ...
            if (res[AppStrings.statusString] == AppConstants.code_200) {
              ...
              Navigator.pushNamedAndRemoveUntil(
                  event.context, RouteDefine.returnListScreen.name, ModalRoute.withName(RouteDefine.returnListScreen.name));
            } else {
              CustomSnackBar.showSnackBar(
        ...
        try {
          ...
          if (resModel.status == AppConstants.code_201) {
            ...
            Navigator.pushReplacementNamed(event.context, RouteDefine.createProductReturnListScreen.name,
                arguments: {'list': state.returnProductList, AppStrings.isUpdateParamString: false, 'status': state.isFromPending});
          } else if (resModel.status == AppConstants.code_403) {
            emit(state.copyWith(isShimmer: false));
            await showDialog(
                context: event.context,
                builder: (_) =>
                    CallAgentDialog(phoneNumber: resModel.agentPhoneNumber ?? '', message: res[AppStrings.messageString], language: state.language));
          } else {
            CustomSnackBar.showSnackBar(
                context: event.context,
                title: AppStrings.getLocalizedStrings(res[AppStrings.messageString], event.context),
        ...
        try {
          ...
          final res = await DioClient(event.context)
              .post('${AppUrlEndPoints.updateReturnUrl}${state.returnProductList.first.returnId}', data: reqModel.toJson());
          CreateReturnResModel resModel = CreateReturnResModel.fromJson(res);
          if (resModel.status == AppConstants.code_201) {
            List<ReturnProduct> list = [];
            if (resModel.data!.first.returnproducts != null) {
            ...
            Navigator.pushReplacementNamed(event.context, RouteDefine.createProductReturnListScreen.name,
                arguments: {'list': list, AppStrings.isUpdateParamString: false, 'status': state.isFromPending});
          } else if (resModel.status == AppConstants.code_403) {
            emit(state.copyWith(isShimmer: false));
            await showDialog(
                context: event.context,
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: AlertDialog(
          ...
          content: Text(AppStrings.getLocalizedStrings(message, context),
              ...
              InkWell(
                ...
                child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Text(AppLocalizations.of(context)!.call_the_agent,
