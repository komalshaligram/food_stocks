  ...
  SplashBloc() : super(SplashState.initial()) {
    on<SplashEvent>((event, emit) async {
      if (event is _splashLoadedEvent) {
        ...
        final startupArguments = preferences.getUserLoggedIn() ? {AppStrings.pushNavigationString: event.pushNavigation} : null;
        await Future.delayed(const Duration(seconds: 1));
        emit(state.copyWith(isAnimate: true));

        await Future.delayed(const Duration(seconds: 2));
        emit(state.copyWith(isRedirected: true, startupArguments: startupArguments));
      }
    });
  }
}
