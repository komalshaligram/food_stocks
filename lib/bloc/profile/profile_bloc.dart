...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  String _resolveSelectedBusinessType({required List<ClientType> businessTypeList, required String placeholder, String? preferredType}) {
    if (preferredType != null && preferredType.isNotEmpty && businessTypeList.any((type) => type.businessType == preferredType)) {
      return preferredType;
    }

    final preserved = state.selectedBusinessType;
    if (preserved.isNotEmpty && preserved != placeholder && businessTypeList.any((type) => type.businessType == preserved)) {
      return preserved;
    }
    return placeholder;
  }
  ...
  void _applyClientTypeId(Emitter<ProfileState> emit, String? clientTypeId) {
    ...
    for (final businessType in state.businessTypeList) {
      if (businessType.id == clientTypeId && businessType.businessType != null) {
    ...
    on<ProfileEvent>((event, emit) async {
      ...
      if (event is _pickProfileImageEvent) {
        ...
        if (pickedFile != null) {
          ...
          try {
            ...
            CustomSnackBar.showSnackBar(
                context: event.context, title: AppLocalizations.of(event.context)!.please_enter_email, type: SnackBarType.failure);
          } catch (e) {
            emit(state.copyWith(isFileUploading: false, isUploadingProcess: false));
          }
        }
        ...
        try {
          ...
          final placeholder = AppLocalizations.of(event.context)!.type_of_business;
          ...
          if (response.status == AppConstants.code_200) {
            emit(state.copyWith(
                ...
                selectedBusinessType: _resolveSelectedBusinessType(businessTypeList: list, placeholder: placeholder)));
        ...
        if (state.isUpdate || preferences.getRegistrationIncomplete()) {
          ...
          try {
            ...
            if (response.status == AppConstants.code_200) {
              ...
              _pendingClientTypeId = response.data?.clients?.first.clientDetail?.clientTypeId;
