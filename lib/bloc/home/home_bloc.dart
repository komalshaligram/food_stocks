      ...
      if (event is _getPreferencesDataEvent) {
            ...
            if (response.product?.isNotEmpty ?? false) {
              ...
              if (event.isBarcode) {
                ...
                productStockList[0][0] = productStockList[0][0].copyWith(
                  ...
                  stock: (productDetailData?.supplierSales?.first.productStock
                          .toString() ??
                      '0'),
                  maxQty: (productDetailData?.sale?.isSale ?? false)
                      ? int.parse(
                          productDetailData?.sale?.saleMaxQuantity ?? '0')
                      : 0,
                );
              } else {
              ...
              if (!preferences.getGuestUser()) {
                try {
                  ...
                  GetAllCartResModel response =
                      GetAllCartResModel.fromJson(res);
                  if (response.status == AppConstants.code_200) {
              ...
              supplierList.addAll(productDetailData?.supplierSales
                      ?.map((supplier) {
                    return ProductSupplierModel(
                      ...
                      maxQty: (productDetailData.sale?.isSale ?? false)
                          ? int.parse(productDetailData.sale?.saleMaxQuantity
                                  .toString() ??
                              '')
                          : 0,
              ...
              if (response.product!.isNotEmpty) {
                add(HomeEvent.relatedProductsEvent(
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            final stockList = productSaleList.map((saleProduct) {
              ...
              return ProductStockModel(
                maxQty: (saleProduct.sale?.isSale ?? false)
                    ? int.parse(saleProduct.sale?.saleMaxQuantity ?? '0')
                    : 0,
                productId: productId,
            ...
            emit(state.copyWith(
                isProductSaleShimmering: false, allShimmering: false));
          }
        } catch (_) {
          emit(state.copyWith(
              isProductSaleShimmering: false, allShimmering: false));
          ...
          try {
            ...
            if (response.status == AppConstants.code_201) {
              ...
              if (event.context.mounted) {
                BlocProvider.of<BottomNavBloc>(event.context).add(
                    BottomNavEvent.updateCartCountEvent(
                        context: event.context));
              }
              CustomSnackBar.showSnackBar(
                context: event.context,
              ...
              productStockList[state.productListIndex]
                ...
                productSupplierIds: state
                    .productStockList[state.productListIndex]
                        [state.productStockUpdateIndex]
                    .productSupplierIds,
                totalPrice: productStockList[state.productListIndex]
                        [state.productStockUpdateIndex]
                    .totalPrice,
                productSaleId: productStockList[state.productListIndex]
                        [state.productStockUpdateIndex]
                    .productSaleId,
              );
              emit(state.copyWith(
        ...
        emit(state.copyWith(
            cartCount: preferences.getCartCount(), isCartCountChange: true));
        ...
        try {
          ...
          final res = await DioClient(event.context)
              .post(AppUrlEndPoints.getOrdersCountUrl, data: reqMap);
        ...
        try {
          final res = await DioClient(event.context).post(
              ...
              data:
                  const GetMessagesReqModel(pageNum: 1, pageLimit: 2).toJson());
          if (res != null) {
        ...
        if (event.isRead) {
          if (messageList[messageList.indexOf(messageList
            ...
            await preferences.setMessageCount(
                count: preferences.getMessageCount() - 1);
        ...
        if (preferences.getGuestUser()) return;
        try {
          final res = await DioClient(event.context).post(
              AppUrlEndPoints.getProfileDetailsUrl,
              data:
                  ProfileDetailsReqModel(id: preferences.getUserId()).toJson(),
              options: Options(
                headers: {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            preferences.setAvailableAllPayment(
                isAvailableAllPayment:
                    clientData?.clientDetail?.isAvailableAllPayments ?? false);
            preferences.setPaymentMethod(
                method: clientData?.clientDetail?.paymentType ?? '');
            preferences.setClientDataOnApp(
                showClientDataOnApp:
            ...
            preferences.setPaymentMethodTypes(
                methods: clientData?.clientDetail?.availablePaymentTypes ?? []);
            preferences.setPaymentMethodCount(
                count: clientData?.clientDetail?.availablePaymentTypes.length
                        .toString() ??
                    '0');
            preferences.setBusinessName(
                businessName: clientData?.clientDetail?.bussinessName ?? '');
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            final stockList = response.data?.map((recommendationProduct) {
              ...
              return ProductStockModel(
                maxQty: (recommendationProduct.sale?.isSale ?? false)
                    ? int.parse(
                        recommendationProduct.sale?.saleMaxQuantity ?? '0')
                    : 0,
                productId: productId,
        ...
        try {
          ...
          emit(state.copyWith(
              isSearching: true, bottlePrice: preferences.getBottleTax()));
          final res = await DioClient(event.context).post(
        ...
        checker.checkUpdate().then((value) {
          ...
          printData(
              "check here currentVersion ${value.currentVersion}"); //return current app version
          printData(
              "check here newVersion ${value.newVersion}"); //return the new app version
          printData("check here appURL ${value.appURL}"); //return the app url
          printData(
              "check here errorMessage ${value.errorMessage}"); //return error message if found else it will return null
        ...
        if (response.status == AppConstants.code_200) {
          ...
          productStockList[2] = productStockList[2].map((product) {
            return product.copyWith(
              ...
              quantity: event.productId == product.productId
                  ? product.quantity
                  : cartMap[product.productId] ?? 0,
            );
          }).toList();
        ...
        try {
          emit(state.copyWith(
              pesachBannerShimmering: true,
              retryLoading: event.isRetryLoading));
          final res = await DioClient(event.context)
          ...
          if (response.status == AppConstants.code_200) {
            ...
            preferences.setIsAppOnMaintenance(
                isAppOnMaintenance: response.data?.isAppOnMaintenance ?? false);
            emit(state.copyWith(
              ...
              buttonEnglishText:
                  response.data?.dataWebViewSettings?.buttonEnglishText ??
                      state.buttonEnglishText ??
                      '',
              buttonHebrewText:
                  response.data?.dataWebViewSettings?.buttonHebrewText ??
            ...
            emit(state.copyWith(
                pesachBannerShimmering: false, retryLoading: false));
          }
        } on ServerException {
          emit(state.copyWith(
              pesachBannerShimmering: false, retryLoading: false));
        ...
        try {
          ...
          if (event.context.mounted) {
            BlocProvider.of<BottomNavBloc>(event.context).add(
                BottomNavEvent.updateCartCountEvent(context: event.context));
          }
        }
        if (!preferences.getGuestUser()) {
          try {
        ...
        try {
          ...
          SuppliersListResponseModel response =
              SuppliersListResponseModel.fromJson(res);
          if (response.status == AppConstants.code_200) {
            emit(state.copyWith(
