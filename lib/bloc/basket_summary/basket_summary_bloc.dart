        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            if (event.isSupplierSingle == 'No' && (response.data?.data?.any((supplier) => supplier.id == event.orderBySupplierId) ?? false)) {
              ...
              emit(state.copyWith(
                  orderSummaryList: response, tempList: filteredList ?? [], totalSupplier: event.totalSupplier ?? 0, isShimmering: false));
            } else {
              emit(state.copyWith(orderSummaryList: response, tempList: response.data?.data ?? [], isShimmering: false));
            }
            add(BasketSummaryEvent.getSupplierDeliveryScheduleEvent(context: event.context));
          } else {
      ...
      if (event is _orderSendEvent) {
        ...
        try {
          OrderSendReqModel reqMap = OrderSendReqModel(
              products: productReqMap, paymentMethod: event.paymentMethod.isNotEmpty ? event.paymentMethod : preferences.getPaymentMethod());
          ...
          if (response.status == AppConstants.code_201) {
            ...
            emit(state.copyWith(
                ...
                errorString: response.message!.contains('MESSAGE')
                    ? AppStrings.getLocalizedStrings(response.message?.toLocalization() ?? response.message!, event.context)
                    : response.message!));
            CustomSnackBar.showSnackBar(
                context: event.context,
                title: AppStrings.getLocalizedStrings(response.message?.toLocalization() ?? response.message!, event.context),
                ...
                builder: (context1) {
                  return CustomOneButtonDialog(
                      ...
                      positiveOnTap: () async {
                        ...
                        try {
                          final res =
                              await DioClient(event.context).get(path: '${AppUrlEndPoints.getLatestOnthewayOrderUrl}${preferences.getUserId()}');
        ...
        final supplierIds = state.tempList.map((supplier) => supplier.suppliers?.id ?? supplier.id ?? '').toList();
