  ...
  ProductSaleBloc() : super(ProductSaleState.initial()) {
    on<ProductSaleEvent>((event, emit) async {
      ...
      if (event is _getProductSalesListEvent) {
        emit(
          state.copyWith(isIncludedVat: preferences.getIsIncludedVat(), isSaleOn: preferences.getShowSale(), isGuestUser: preferences.getGuestUser(), bottleDeposit: preferences.getBottleTax(), isSubUserAddToBasket: preferences.getCanAddToBasket(), isGridView: preferences.getSalesProductGrid(), language: preferences.getAppLanguage()),
        );

        List<ProductSale> productSaleList = List.from(state.productSalesList, growable: true);
        ...
        try {
          ...
          final res = await DioClient(event.context).post(
            ...
            data: ProductSalesReqModel(pageNum: state.pageNum + 1, pageLimit: AppConstants.saleProductPageLimit, search: state.search).toJson(),
          );
          ...
          if (response.status == AppConstants.code_200) {
            ...
            emit(state.copyWith(
              isBottomOfProducts: productSaleList.length == (response.metaData?.totalFilteredCount ?? 0) ? true : false,
              isProgress: false,
            ));
          } else {
            emit(state.copyWith(
            ...
            if (response.product!.isNotEmpty) {
              ...
              if (supplierList.isNotEmpty) {
                ...
                if (!isSupplierSelected || state.productListIndex == 0) {
                  ...
                  add(ProductSaleEvent.supplierSelectionEvent(supplierIndex: supplierIndex, context: event.context, supplierSaleIndex: supplierSaleIndex));
                }
              }
            } else {
        ...
        if (state.productStockList[event.productListIndex][event.productStockUpdateIndex].quantity == 0) {
          ...
          if (_isProductInCart) {
            ...
            try {
              ...
              InsertCartResModel response = InsertCartResModel.fromJson(res);
              if (response.status == AppConstants.code_201) {
                // if (!state.productStockList[event.productListIndex][event.productStockUpdateIndex].productIsInCart) {
                add(const ProductSaleEvent.setCartCountEvent());
                // }
