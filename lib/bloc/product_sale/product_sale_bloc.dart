    ...
    on<ProductSaleEvent>((event, emit) async {
      ...
      if (event is _getProductSalesListEvent) {
        ...
        List<ProductSale> productSaleList =
            List.from(state.productSalesList, growable: true);
        ...
        if (!state.isProgress) {
          add(ProductSaleEvent.getProductSalesListEvent(
              context: event.context));
        }
      } else if (event is _getGridListView) {
        preferences.setSalesProductGridListView(
            isSalesProductGrid: !state.isGridView);
        emit(state.copyWith(isGridView: !state.isGridView));
      } else if (event is _getProductDetailsEvent) {
        add(const ProductSaleEvent.removeRelatedProductEvent());
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            if (response.product!.isNotEmpty) {
              ...
              if (response.product!.isNotEmpty) {
                add(ProductSaleEvent.relatedProductsEvent(
                    context: event.context,
                    productId: response.product?.first.id ?? ''));
              }
              ...
              if (supplierList.isNotEmpty) {
                ...
                if (!isSupplierSelected || state.productListIndex == 0) {
                  ...
                  add(ProductSaleEvent.supplierSelectionEvent(
            ...
            if (response.status == AppConstants.code_201) {
              if (!state
                  .productStockList[state.productListIndex]
                      [state.productStockUpdateIndex]
                  .productIsInCart) {
                add(const ProductSaleEvent.setCartCountEvent());
              }
              Vibration.vibrate();
              List<List<ProductStockModel>> productStockList =
                  state.productStockList.toList(growable: true);
