  ...
  ProductSaleBloc() : super(ProductSaleState.initial()) {
    on<ProductSaleEvent>((event, emit) async {
      ...
      if (event is _getProductSalesListEvent) {
        ...
        try {
          ...
          if (response.status == AppConstants.code_200) {
            ...
            emit(state.copyWith(
                ...
                isProgress: false));
            emit(state.copyWith(
                isBottomOfProducts: productSaleList.length == (response.metaData?.totalFilteredCount ?? 0) ? true : false, isProgress: false));
          } else {
        ...
        emit(state.copyWith(
            pageNum: 0, isProgress: false, productSalesList: [], productStockList: [state.productStockList[0], [], []], isBottomOfProducts: false));
        if (!state.isProgress) {
